...
test(`${ID} bundles to a module that reaches only what it needs, and starts on a page`, async () => {
  ...
  assert.deepEqual(Object.values(map).sort(), ["keel/core"]);
  ...
  assert.equal(api["contract"], "ai/animal@1.0.0");
  assert.deepEqual([...(api["drives"] as () => string[])()], ["packs/animals@1.0.0"], "bound to the quadruped pack by contract");
});
