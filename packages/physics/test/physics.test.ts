...
// with no tunnelling, wedges, and determinism.
...
import { boxDistance, nearestOnRail, slopeOf, solidDistance, wedgeDistance } from "../src/solids.ts";
import type { Box, Wedge } from "../src/solids.ts";

const DT = 1 / 120;
const K = TUNING;
const dot = (a: Vec3Like, b: Vec3Like): number => a[0] * b[0] + a[1] * b[1] + a[2] * b[2];
const FLOOR: Box = { c: [0, -1, 0], h: [60, 1, 60] }; // (top at y = 0)
const slab = (x0: number, x1: number, z0: number, z1: number, y = 0, mat = 1): Box => ({ c: [(x0 + x1) / 2, y - 1.5, (z0 + z1) / 2], h: [(x1 - x0) / 2, 1.5, (z1 - z0) / 2], mat });
const idle: BodyInput = { move: [0, 0], jump: false, hold: false };
const run = (body: Character, n: number, input: BodyInput | ((b: Character, i: number) => BodyInput)): BodyEvent["type"][] => {
...
const settle = (body: Character): BodyEvent["type"][] => run(body, 60, idle);
const has = (b: Character, type: BodyEvent["type"]): boolean => b.events.some((e) => e.type === type);
