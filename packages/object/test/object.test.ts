// The port of the proof of concept's tests/object.test.mjs: definitions,
// placement, colliders, sockets, resting, baking, the catalogue -- and the
// fix: an instance's own tags survive a move and a settle.

import { test } from "node:test";
import assert from "node:assert/strict";
import { localToWorld, seedFromToken, wrapAngle } from "@keel-engine/core";
import type { Vec3 } from "@keel-engine/core";
import { boxDistance, createCharacter } from "@keel-engine/physics";
import { angleBetween, createRegistry, detectFront, distance, mk, sphere, toWorld } from "@keel-engine/scene";
import {
  bakeForPhysics, bakeForRenderer, bottomOf, defineObject, footprint, frontOfObject, movedObject, onSocket, placeObject,
  restsOn, settle, socketOf, socketsOfKind, topsOf, worldAabb, worldColliders, worldRails, worldSockets, yawToShow,
} from "../src/object.ts";
import { PIECE_KEYS, buildPiece, buildPieceFrom, pieceStream, registerCatalogue } from "../src/catalogue.ts";
import { boxPart, toPart } from "../src/index.ts";
import { PIECE_MATS, levelOf } from "./wallrun-level.ts";
...
const nearV = (a: readonly number[], b: readonly number[], eps = 1e-9, msg = ""): void => a.forEach((v, i) => near(v, b[i]!, eps, `${msg}[${i}]`));
const SEEDS = [1, 2, 3].map((i) => seedFromToken(i, "objects")) as [string, string, string];
...
const desk = () => defineObject({
  ...
  parts: [
    ...
    ...([[-1, -1], [1, -1], [-1, 1], [1, 1]] as const).map(([x, z]) => ({ box: { c: [x * 0.55, 0.345, z * 0.3] as Vec3, h: [0.03, 0.345, 0.03] as Vec3 }, name: "leg", mat: "wood" })),
...
test("object: a definition keeps its parts, colliders, bounds, front and sockets", () => {
  ...
  // @ts-expect-error -- (not a rest mode)
  assert.throws(() => defineObject({ key: "x", parts: [{ box: { c: [0, 0, 0], h: [1, 1, 1] } }], rest: "sit" }));
  assert.throws(() => defineObject({ key: "x", parts: [{ name: "?" } as never] }));
});
...
test("object: placement -- world bounds, footprint, colliders and sockets move with the instance", () => {
  ...
  for (const yaw of YAWS) {
    ...
    d.colliders.forEach((b, i) => {
      nearV(cs[i]!.c, localToWorld([4, 1, -3], yaw, b.c.map((v) => v * 1.5) as Vec3), 1e-9);
    ...
    const dt = d.sockets["top"]!;
    const onTop = toWorld(inst, [top.extent![0] / 1.5 * 0.5, dt.pos[1], dt.pos[2]]);
    assert.ok(onSocket(inst, "top", onTop));
    assert.ok(!onSocket(inst, "top", toWorld(inst, [2, dt.pos[1], dt.pos[2]])));
...
test("object: colliders match their parts (physics' boxDistance == the parts' SDF)", () => {
  for (const key of ["pillar", "wall", "pad", "ramp", "stairs", "arch", "tunnel"] as const) {
    for (const seed of SEEDS) {
      ...
      for (const yaw of [0.3, 2.2]) {
        ...
        for (let i = 0; i < 60; i += 1) {
          const u = [((i * 0.618034) % 1), ((i * 0.381966 + 0.2) % 1), ((i * 0.7548776 + 0.5) % 1)] as const;
          const p: Vec3 = [b[0] - 0.2 * size + u[0] * 1.4 * size, b[1] - 0.2 * size + u[1] * 1.4 * size, b[2] - 0.2 * size + u[2] * 1.4 * size];
          ...
          if (sdf > 0) near(phys, sdf, 1e-9, `${key} at ${String(p)}`);
...
test("object: baking for the renderer and for physics", () => {
  ...
  const both = [rail, lamp];
  const r = bakeForRenderer(both, { mats });
  const nCaps = both.reduce((s, i) => s + i.def.parts.filter((p) => p.prim?.type === "capsule").length, 0);
  const nBoxes = both.reduce((s, i) => s + i.def.parts.filter((p) => p.prim?.type === "box").length, 0);
  ...
  assert.equal(p0.type, "capsule");
  if (p0.type === "capsule") nearV(r.capsules[0]!.a, toWorld(rail, p0.a), 1e-12);
  // Physics: the rail's line in the world, the lamp's solid pole and base.
  const ph = bakeForPhysics(both, { mats });
...
test("object: the catalogue -- every piece, deterministic, fronts declared and never contradicted", () => {
  ...
  for (const key of ["sign", "lampPost", "crate"] as const) assert.equal(detectFront(buildPiece(key, SEEDS[0])).agrees, true, key);
...
test("object: turning a thing to show a viewer its front (or its declared show side)", () => {
  ...
  for (const yaw of YAWS) {
    const eye: Vec3 = [Math.sin(yaw) * 6, 1.5, Math.cos(yaw) * 6];
    const pos: Vec3 = [0.5, 0, -0.3];
  ...
  const v = pig.sockets["view"]!.pos;
  assert.ok(Math.abs(v[0] - Math.sin(0.6) * Math.hypot(v[0], v[2])) < 1e-9, "the view point is on the show side");
});

test("object: WALLRUN's course is built from catalogue pieces, and its props rest on the plaza", () => {
...
test("object: prims are scene's, re-exported (one implementation)", () => {
  const p = toPart({ box: { c: [0, 1, 0], h: [1, 1, 1] }, name: "b" });
  assert.equal(p.sdf(0, 3, 0), 1);
  assert.deepEqual(boxPart({ c: [0, 0, 0], h: [1, 2, 3] }).bounds, [-1, -2, -3, 1, 2, 3]);
});

// ---------------------------------------------------------------- the fix

test("fix: movedObject and settle keep the instance's own tags (the proof of concept dropped them)", () => {
  const pad = placeObject(buildPiece("pad", SEEDS[0]), { pos: [0, 0, 0] });
  const crate = placeObject(buildPiece("crate", SEEDS[1]), { pos: [0, 3, 0], tags: ["loot", "blue"] });
  assert.deepEqual(crate.tags, ["blue", "loot", "movable", "prop", "solid"]);
  const moved = movedObject(crate, { pos: [1, 3, 0], yaw: 0.5 });
  assert.deepEqual(moved.tags, crate.tags, "moved: its tags kept");
  assert.equal(moved.id, crate.id);
  const r = settle(crate, [pad]);
  assert.ok(r.rests);
  assert.deepEqual(r.instance.tags, crate.tags, "settled: its tags kept");
  // (Give tags to replace them; with none of its own it moves exactly as the proof of concept's did.)
  assert.deepEqual(movedObject(crate, { tags: ["red"] }).tags, ["movable", "prop", "red", "solid"]);
  const plain = placeObject(crate.def, { pos: [0, 3, 0], id: "plain" });
  assert.deepEqual(movedObject(plain, { pos: [2, 2, 2] }).tags, crate.def.tags);
});
