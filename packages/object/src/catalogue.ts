...
// ported from the proof of concept's src/object/catalogue.js;
// test/poc-equality.test.ts proves every piece identical over many seeds and
// sizes: pillar, wall, pad, ramp, rail, arch, tunnel, crate, bench, sign, lamp
// post, stairs. Each is built in its own frame (+z front, +y up, the pivot in
// the middle of its base), from boxes and capsules -- the solids the renderer
// and the character controller share -- so its colliders ARE its parts.
//
//   build(S, ctx) -> definition      S: a seeded stream (@keel-engine/core stream)
...
//
// Fronts, as declared (and each checked by test/object.test.ts):
...
import { defineObject } from "./object.ts";
import type { Mat, ObjectDef } from "./object.ts";

// A number from ctx, or drawn -- the draw ALWAYS happens.
const pick = (S: Stream, given: number | null | undefined, a: number, b: number): number => { const v = S.between(a, b); return given ?? v; };
const pickInt = (S: Stream, given: number | null | undefined, a: number, b: number): number => { const v = S.int(a, b); return given ?? v; };
const chance = (S: Stream, given: boolean | null | undefined, p: number): boolean => { const v = S.chance(p); return given ?? v; };

interface PartExtra { readonly yaw?: number; readonly mat?: Mat; readonly collide?: boolean }
const bx = (name: string, c: Vec3Like, h: Vec3Like, extra: PartExtra = {}): PartLike => ({ box: { c, h, yaw: extra.yaw ?? 0 }, name, mat: extra.mat ?? "wall", ...extra }) as PartLike;
const cap = (name: string, a: Vec3Like, b: Vec3Like, r: number, extra: PartExtra = {}): PartLike => ({ capsule: { a, b, r }, name, mat: extra.mat ?? "metal", ...extra }) as PartLike;

// ---------------------------------------------------------------- what each piece takes and keeps

/** Sizes each piece can be given (anything left out is drawn). `sink` pushes solids below the pivot. */
export interface PieceContexts {
  pillar: { w?: number; d?: number; h?: number; sink?: number };
  wall: { length?: number; h?: number; thick?: number; sink?: number };
  pad: { w?: number; d?: number; h?: number; sink?: number; lip?: boolean };
  ramp: { w?: number; length?: number; h?: number; steps?: number };
  stairs: { w?: number; steps?: number; rise?: number; tread?: number };
  /** shape "ease": sin² (leaves and arrives straight and level, as WALLRUN's does); "arc": sin. */
  rail: { length?: number; bend?: number; rise?: number; y?: number; segments?: number; r?: number; posts?: boolean; shape?: "ease" | "arc" };
  arch: { span?: number; h?: number; thick?: number; post?: number };
  tunnel: { length?: number; span?: number; h?: number; thick?: number; roof?: number; floor?: boolean };
  crate: { size?: number; tall?: number };
  bench: { w?: number; seatH?: number; seatD?: number; back?: boolean; backH?: number };
  sign: { w?: number; h?: number; lift?: number; twoPosts?: boolean; glow?: boolean };
  lampPost: { h?: number; reach?: number; r?: number };
}
/** What each piece keeps in its meta. */
export interface PieceMetas {
  pillar: { w: number; d: number; h: number };
  wall: { length: number; h: number; thick: number };
  pad: { w: number; d: number; h: number };
  ramp: { w: number; length: number; h: number; steps: number };
  stairs: { w: number; steps: number; rise: number; tread: number };
  rail: { length: number; bend: number; rise: number };
  arch: { span: number; h: number; thick: number };
  tunnel: { length: number; span: number; h: number };
  crate: { size: number; tall: number };
  bench: { w: number; seatH: number; hasBack: boolean };
  sign: { w: number; h: number; lift: number; glow: boolean };
  lampPost: { h: number; reach: number };
}
export type PieceKey = keyof PieceContexts;
/** A piece's definition, with its meta typed. */
export type PieceDef<K extends PieceKey> = ObjectDef<PieceMetas[K]>;
export type PieceBuilder<K extends PieceKey> = (S: Stream, ctx?: PieceContexts[K]) => PieceDef<K>;

// ---------------------------------------------------------------- the pieces

function pillar(S: Stream, ctx: PieceContexts["pillar"] = {}): PieceDef<"pillar"> {
...
function wall(S: Stream, ctx: PieceContexts["wall"] = {}): PieceDef<"wall"> {
...
function pad(S: Stream, ctx: PieceContexts["pad"] = {}): PieceDef<"pad"> {
...
// Rises from its foot at +z to its top at -z, in slabs (the catalogue's
// ramps are stairs of slabs: a wedge ramp is @keel-engine/physics' own solid).
function ramp(S: Stream, ctx: PieceContexts["ramp"] = {}): PieceDef<"ramp"> {
...
function stairs(S: Stream, ctx: PieceContexts["stairs"] = {}): PieceDef<"stairs"> {
...
function rail(S: Stream, ctx: PieceContexts["rail"] = {}): PieceDef<"rail"> {
...
function arch(S: Stream, ctx: PieceContexts["arch"] = {}): PieceDef<"arch"> {
...
function tunnel(S: Stream, ctx: PieceContexts["tunnel"] = {}): PieceDef<"tunnel"> {
...
function crate(S: Stream, ctx: PieceContexts["crate"] = {}): PieceDef<"crate"> {
...
function bench(S: Stream, ctx: PieceContexts["bench"] = {}): PieceDef<"bench"> {
...
function sign(S: Stream, ctx: PieceContexts["sign"] = {}): PieceDef<"sign"> {
...
function lampPost(S: Stream, ctx: PieceContexts["lampPost"] = {}): PieceDef<"lampPost"> {
...
/** A catalogue entry: key, weight, role ("level" or "prop"), build. */
export type Piece = { [K in PieceKey]: readonly [K, number, "level" | "prop", PieceBuilder<K>] }[PieceKey];
...
export const PIECE_KEYS: readonly PieceKey[] = PIECES.map(([k]) => k);

/**
 * Put the catalogue into a registry as the realm "Objects" (the default
 * registry when none is given). Returns the realm.
 */
export function registerCatalogue(registry: Registry = defaultRegistry, realmName = "Objects"): Realm {
  const realm = registry.defineRealm(realmName);
  for (const [key, weight, role, build] of PIECES) if (!realm.get(key)) realm.add({ key, weight, role, build: build as unknown as Builder });
  return realm;
}
...
/** A piece made from a seed: its definition, stamped with the realm and seed. */
export type BuiltPiece<K extends PieceKey> = PieceDef<K> & { realm: string; seed: unknown };
 ...
 */
export function buildPiece<K extends PieceKey>(key: K, seed: unknown, ctx: PieceContexts[K] = {}): BuiltPiece<K> {
  if (!PIECE_KEYS.includes(key)) throw new RangeError(`No piece ${String(key)}. Pieces: ${PIECE_KEYS.join(", ")}.`);
  return OWN.makeAsset(seed, { realm: "Objects", key, ctx }) as unknown as BuiltPiece<K>;
}

/** A builder from a stream you already have (a level's own stream). */
export function buildPieceFrom<K extends PieceKey>(key: K, S: Stream, ctx: PieceContexts[K] = {}): PieceDef<K> {
  const e = PIECES.find(([k]) => k === key);
  if (!e) throw new RangeError(`No piece ${String(key)}.`);
  return (e[3] as unknown as PieceBuilder<K>)(S, ctx);
}

/** A stream for a seed and a label (a level's rooms, a prop set), for buildPieceFrom. */
export const pieceStream = (seed: unknown, slot = 1): Stream => stream(createRoll(seed), slot);
