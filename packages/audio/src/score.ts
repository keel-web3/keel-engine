...
//
// NOCTURNES' own music is scoreOf(moodOfNocturnes(g)) (./nocturnes.ts), equal
// to its scoreOf(g) byte for byte (test/equality.test.ts).
//
// (Ported from the proof of concept's src/audio/score.js: same draws, in the
// same order, and the plan's keys in the same order, so its JSON is the same.)

// ---------------------------------------------------------------- types

export type ModeName = "ionian" | "dorian" | "phrygian" | "lydian" | "mixolydian" | "aeolian" | "harmonic";
export type KeysName = "rhodes" | "wurli" | "piano" | "felt" | "vibes" | "organ" | "guitar";
export type LeadName = "rhodes" | "piano" | "celesta" | "kalimba" | "musicbox" | "vibes" | "guitar" | "chip";
export type BassName = "sub" | "upright" | "electric" | "synth";
export type KitName = "dusty" | "boombap" | "brushed" | "soft" | "rim" | "chip";
export type WeatherKind = "rain" | "waves" | "traffic" | "wind" | "hush" | "crickets" | "car" | "chimes" | "shimmer";
export type RoomKind = "vinyl" | "crackle" | "fan" | "hum";
export type SectionName = "A" | "B" | "A2" | "C" | "T";
export type CompStyle = "strum" | "arp" | "stabs" | "sustain" | "push";
export type BassStyle = "root" | "rootfifth" | "walk" | "sync";
export type BackBeat = "snare" | "rim" | "clap" | "both";
export type Spread = "close" | "open" | "drop2";

/** [choice, weight] pairs: a weighted draw. */
export type Weighted<T> = readonly (readonly [T, number])[];

/** A band: who may play, the tempo and swing it may take, its room, jazz, voice, scratch, brightness and wear. */
export interface Band {
  readonly keys: Weighted<string>;
  readonly lead: Weighted<string>;
  readonly bass: Weighted<string>;
  readonly feel: Weighted<string>;
  readonly tempo: readonly [number, number];
  readonly swing: readonly [number, number];
  readonly room: number;
  readonly jazz: number;
  readonly vox: number;
...
/** Any band fields, laid over "Loose". */
export type BandInput = { readonly [K in keyof Band]?: Band[K] | undefined };

/** Choices fixed by hand; the seed decides the rest. */
export interface Pins {
  readonly band?: string | undefined;
  readonly keys?: string | undefined;
  readonly lead?: string | undefined;
  readonly bass?: string | undefined;
  readonly kit?: string | undefined;
  readonly mode?: string | undefined;
  /** 0-11 (or "0"-"11"), or a note name ("C#"). */
  readonly key?: string | number | undefined;
  /** A bpm (or a picture-locked tempo nearest it). */
  readonly tempo?: string | number | undefined;
}

/** What a record is written from (all optional). */
export interface Mood {
  /** The seed, when scoreOf is given none. */
  readonly seed?: string | number | undefined;
  /** What the plan calls its band (NOCTURNES: the theme). */
  readonly name?: string | undefined;
  /** A band preset's name (BANDS) or a band object. */
  readonly band?: string | BandInput | undefined;
  /** 0-360: the key round the circle of fifths (none: the seed picks). */
  readonly hue?: number | undefined;
  /** The mode the band leans to when it has none of its own. */
  readonly mode?: string | undefined;
  /** A darker record (harmonic minor). */
  readonly eclipse?: boolean | undefined;
  /** Seconds of a picture's own loop: the tempo locks so the music is whole loops of it. */
  readonly picture?: number | undefined;
  /** A label for what's outside. */
  readonly view?: string | undefined;
  /** Layers outside (WEATHER_KINDS). */
  readonly weather?: readonly string[] | undefined;
  /** Extra room layers (ROOM_KINDS). */
  readonly room?: readonly string[] | undefined;
  /** 0-1, how open the record's top is. */
  readonly dark?: number | undefined;
  /** Extra reverb over the band's room. */
  readonly space?: number | undefined;
  /** Carried on the plan: the player's default intensity. */
  readonly energy?: number | undefined;
  readonly pins?: Pins | undefined;
}

/** A chord as semitones over the tonic: its root and its 3rd, 5th, 7th and 9th (deg -1: a secondary dominant, -2: a borrowed iv). */
export interface Chord { readonly root: number; readonly tones: readonly number[]; readonly deg: number }
export interface Bar { readonly sec: SectionName; readonly k: number; readonly of: number; readonly chord: Chord }
...
export interface Motif { readonly rhythm: readonly number[]; readonly moves: readonly number[]; readonly lengths: readonly number[] }
/** The instruments' make. */
export interface Design {
  readonly seed: number;
  readonly bright: number;
  readonly bell: number;
  readonly kickHz: number;
  readonly kickDecay: number;
  readonly punch: number;
  readonly snareBody: number;
  readonly snareTone: number;
  readonly snareDecay: number;
  readonly hatTone: number;
  ...
  readonly back: BackBeat;
}
export interface Mix { readonly cutoff: number; readonly tape: number; readonly reverb: number; readonly wow: number; readonly level: number }
export type Form = readonly (readonly [SectionName, number])[];

/** The music as a plan: plain JSON. The same mood and seed make the same plan everywhere. */
...
/** A seeded stream (exact integer arithmetic: the same in every browser). */
...
export const MINORISH: ReadonlySet<string> = new Set(["dorian", "phrygian", "aeolian", "harmonic"]);
// Where a chord likes to go next (scale degree -> [next, weight]).
const MOVES: Record<"major" | "minor", Record<number, Weighted<number>>> = {
...
/** A band preset by name, or undefined (any string: NOCTURNES' pins may name no band). */
export const bandNamed = (name: string): Band | undefined => (BANDS as Record<string, Band>)[name];

/** What can be chosen (and pinned). */
export const CHOICES = {
  band: Object.keys(BANDS) as BandName[], kit: ["dusty", "boombap", "brushed", "soft", "rim", "chip"] as KitName[], keys: ["rhodes", "wurli", "piano", "felt", "vibes", "organ", "guitar"] as KeysName[],
  lead: ["rhodes", "piano", "celesta", "kalimba", "musicbox", "vibes", "guitar", "chip"] as LeadName[], bass: ["sub", "upright", "electric", "synth"] as BassName[], mode: Object.keys(MODES) as ModeName[],
  key: ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"],
};
/** The layers under the record: outside, and in the room (the vinyl is always there). */
export const WEATHER_KINDS: readonly WeatherKind[] = ["rain", "waves", "traffic", "wind", "hush", "crickets", "car", "chimes", "shimmer"];
export const ROOM_KINDS: readonly RoomKind[] = ["vinyl", "crackle", "fan", "hum"];
...
export function streamOf(text: unknown): Stream {
  ...
  const weighted = <T>(l: Weighted<T>): T => {
    ...
    let r = f() * s;
    for (const [v, w] of l) { if ((r -= w) < 0) return v; }
    return (l[l.length - 1] as readonly [T, number])[0];
  };
  return {
    f,
    between: (x, y) => x + (y - x) * f(),
    int: (x, y) => x + Math.floor(f() * (y - x + 1)),
    pick: <T>(l: readonly T[]): T => l[Math.floor(f() * l.length)] as T,
    chance: (p) => f() < p,
    weighted,
  };
}
...
export function diatonic(mode: ModeName, d: number): Chord {
  const sc = MODES[mode];
  const at = (k: number): number => sc[(d + k) % 7]! + 12 * Math.floor((d + k) / 7);
  return { root: at(0) % 12, tones: [at(2), at(4), at(6), at(8)], deg: d };
}
const dominantOf = (target: Chord): Chord => { const r = (target.root + 7) % 12; return { root: r, tones: [r + 4, r + 7, r + 10, r + 14], deg: -1 }; };
const borrowedIv = (mode: ModeName): Chord => { const r = MODES[mode][3]!; return { root: r, tones: [r + 3, r + 7, r + 10, r + 14], deg: -2 }; };

interface Harmony { readonly family: "major" | "minor"; readonly mode: ModeName; readonly jazz: number }

/** Eight bars of chords, walked from `from`, landing so the next section can begin. */
function progression(S: Stream, plan: Harmony, from: number, len: number, cadence?: number): Chord[] {
...
function motif(S: Stream): Motif {
  ...
  const moves = rhythm.map(() => S.weighted<number>([[1, 3], [-1, 3], [2, 2], [-2, 2], [0, 1], [3, 1], [-3, 1]]));
  return { rhythm, moves, lengths: rhythm.map((s, i) => Math.max(2, (rhythm[i + 1] ?? 32) - s)) };
}

const KICKS: Record<string, readonly (readonly [number, number])[]> = {
...
type HatStyle = "16" | "8" | "4" | "16skip" | "swing16" | "shaker";
const HATS: Record<HatStyle, readonly number[]> = {
  16: [...Array(16).keys()], 8: [0, 2, 4, 6, 8, 10, 12, 14], 4: [0, 4, 8, 12], "16skip": [0, 2, 3, 4, 6, 8, 10, 11, 12, 14], swing16: [0, 2, 4, 6, 7, 8, 10, 12, 14, 15], shaker: [],
};

/** A beat of its own: where this seed's kicks fall, how its hats run, its ghosts, its open hats, a shaker. */
function grooveOf(S: Stream, feel: string): Groove {
  const g: Groove = { kick: [0], back: [4, 12], hat: [], ghost: [], open: [], shaker: [] };
  const kicks = KICKS[feel] ?? [[10, 0.6], [7, 0.4]];
  for (const [s, p] of kicks) if (S.chance(p)) g.kick.push(s);
  if (g.kick.length === 1) g.kick.push(10);
  const style = S.weighted<HatStyle>(feel === "chip" ? [["16", 3], ["8", 1]] : feel === "brushed" || feel === "soft" ? [["shaker", 3], ["4", 2], ["8", 1]] : [["8", 3], ["16skip", 2], ["swing16", 2], ["4", 1]]);
  g.hat = [...HATS[style]];
...
const COMPS: Record<string, Weighted<CompStyle>> = {
...
export interface ScoreOptions { readonly pins?: Pins | undefined }
 ...
 */
export function scoreOf(mood: Mood = {}, seed: string | number | undefined = mood.seed, opts: ScoreOptions = {}): Plan {
  const S = streamOf(`${seed}|music`);
  // (Anything can be picked by hand -- the pins -- and the seed decides the rest.)
  const o: Pins = { ...(mood.pins ?? {}), ...(opts.pins ?? {}) };
  const given = typeof mood.band === "string" ? bandNamed(mood.band) : mood.band;
  const theme = o.band || mood.name || (typeof mood.band === "string" ? mood.band : "Loose");
  const band = { ...BANDS.Loose, ...(o.band ? bandNamed(o.band) ?? {} : given ?? {}) } as Band;
  ...
  let mode = (band.mode ?? mood.mode ?? "aeolian") as ModeName;
  if (band.lean === "major" && MINORISH.has(mode)) mode = S.chance(0.6) ? "lydian" : "ionian";
  if (!band.mode && !band.lean && S.chance(0.3)) mode = S.pick<ModeName>(MINORISH.has(mode) ? ["dorian", "aeolian"] : ["ionian", "mixolydian", "lydian"]); // (a seed's own turn)
  if (mood.eclipse) mode = "harmonic"; // (a darker record)
  if (o.mode && (MODES as Record<string, unknown>)[o.mode]) mode = o.mode as ModeName;
  ...
  let tempo: { bpm: number; bars: number[] };
  if (typeof picture === "number") {
    const options = ([[5, 4], [4, 3], [3, 2], [8, 5], [5, 3], [2, 1], [9, 4], [5, 2], [8, 3], [3, 1]] as const)
    ...
    const nearest = (want: number) => options.reduce((best, r) => (Math.abs(r.bpm - want) < Math.abs(best.bpm - want) ? r : best));
  ...
  const swing = S.between(band.swing[0], band.swing[1]);
  const loopBars = form.reduce((a, [, n]) => a + n, 0);
  // How they play: the keys' comping, the bass's line, the voicing, the groove.
  const comp = S.weighted<CompStyle>(COMPS[keys] ?? [["stabs", 3], ["push", 2], ["sustain", 2], ["arp", 1]]);
  const bassStyle = S.weighted<BassStyle>([["root", 2], ["rootfifth", 3], ["walk", bass === "upright" || band.jazz > 0.4 ? 4 : 1], ["sync", bass === "synth" || bass === "electric" ? 3 : 1]]);
  const groove = grooveOf(S, kit);
  ...
  const design: Design = {
    ...
    pump: S.between(0.3, 0.65), strum: S.between(0.004, 0.028), spread: S.pick<Spread>(["close", "open", "drop2"]),
    back: kit === "rim" ? "rim" : kit === "brushed" ? S.pick<BackBeat>(["snare", "rim"]) : S.weighted<BackBeat>([["snare", 4], ["clap", 1], ["both", 2]]),
  };
  // The chords: A and B walked from the seed; A2 is A with a turn or two; C sits; T turns round home.
  const harmony: Harmony = { family, mode, jazz: band.jazz };
  const len = (name: SectionName) => Math.max(...form.filter(([n]) => n === name).map(([, n]) => n), 0);
  const A = progression(S, harmony, 0, 8);
  const B = progression(S, harmony, family === "major" ? S.pick([3, 5]) : S.pick([3, 5, 2]), 8);
  const A2 = A.map((c, i) => (i % 4 === 3 && S.chance(0.5) ? progression(S, harmony, c.deg < 0 ? 0 : c.deg, 2)[1]! : c));
  ...
  const T = progression(S, harmony, family === "major" ? 1 : 3, Math.max(4, len("T")));
  T[T.length - 1] = family === "major" ? diatonic(mode, 4) : diatonic(mode, 6); // (it leads back to the top)
  const byName: Record<SectionName, Chord[]> = { A, B, A2, C, T };
  const barList = form.flatMap(([name, n]) => byName[name].slice(0, n).map((chord, k): Bar => ({ sec: name, k, of: n, chord })));
  const tune = motif(S);
  // What's outside, and what's in the room: the record's own crackle is always there.
  const weather = [...(mood.weather ?? ["hush"])];
  const room = ["vinyl", ...(mood.room ?? []).filter((r) => r !== "vinyl")];
  const dark = mood.dark ?? 0.72;
  const mix: Mix = {
    cutoff: Math.round((2400 + 3600 * dark) * (0.85 + 0.3 * design.bright)), tape: Math.min(0.5, 0.1 + 0.3 * band.worn + S.between(0, 0.06)),
    reverb: Math.min(0.6, band.room + (mood.space ?? 0)), wow: S.between(0.1, 0.35) * (0.6 + band.worn),
    level: -4,
  };
  // (The keys in the proof of concept's order: its JSON is NOCTURNES' byte for byte.)
  const plan: Plan = {
    seed: String(seed), theme, view: mood.view ?? "Stars", eclipse: Boolean(mood.eclipse), mode, family, tonic,
    bpm: Math.round(bpm * 1000) / 1000, barSec, swing, jazz: band.jazz,
    keys, lead, bass, kit, vox: band.vox, scratch: band.scratch,
    form, loopBars,
    loopSec: loopBars * barSec,
    picture: typeof picture === "number" ? picture : null, // (seconds of the picture's own loop: the music's is a whole number of them)
    comp, bassStyle, groove, design, bars: barList, motif: tune, weather, room, mix,
    ...(mood.energy !== undefined ? { energy: mood.energy } : {}), // (games: where intensity starts)
  };
  return plan;
}
...
const W = <T>(pairs: readonly (readonly [T, number])[]): [T, number][] => pairs.filter(([, w]) => w > 0.001).map(([v, w]) => [v, Math.round(w * 1000) / 1000]);
const clamp01 = (x: unknown): number => Math.max(0, Math.min(1, Number.isFinite(+(x as number)) ? +(x as number) : 0.5));

/** A few words about a game. */
export interface MoodSpec {
  /** 0 idle, 1 driving. */
  readonly energy?: number;
  /** 0 day, 1 deep night. */
  readonly darkness?: number;
  /** Layers under the record (WEATHER_KINDS); one name or a list. */
  readonly weather?: string | readonly string[];
  /** A bpm to pin, or [lo, hi]. */
  readonly tempo?: number | readonly [number, number];
  /** The palette's key hue: the music's key. */
  readonly hue?: number | undefined;
...
/** A mood as moodFor makes it (a band of its own, the rest filled in). */
export interface GameMood extends Mood {
  readonly name: string;
  readonly band: BandInput;
  readonly mode: ModeName;
  readonly hue: number | undefined;
  readonly view: string;
  readonly weather: readonly string[];
 ...
 */
export function moodFor({ energy = 0.5, darkness = 0.6, weather = ["hush"], tempo, hue, name = "Game", room = [], space = 0, pins = {} }: MoodSpec = {}): GameMood {
  const e = clamp01(energy);
  const k = clamp01(darkness);
  const lo = Math.round(64 + 18 * e);
  const hi = Math.min(96, Math.round(lo + 12 + 4 * e));
  const band: { -readonly [K in keyof Band]?: Band[K] } = {
  ...
  const mode: ModeName = k > 0.62 ? "aeolian" : k > 0.4 ? "dorian" : k > 0.2 ? "mixolydian" : "ionian"; // (the seed turns it a third of the time)
  const layers = ((Array.isArray(weather) ? weather : [weather]) as readonly string[]).filter((w) => (WEATHER_KINDS as readonly string[]).includes(w));
  return {
    name, band, mode, hue, view: name, weather: layers.length ? layers : ["hush"], room: room.filter((r) => (ROOM_KINDS as readonly string[]).includes(r)),
