...
//
// (Every operation in the proof of concept's order: the arrays are its arrays, bit for bit.)

import type { Design, Plan } from "./score.ts";

/** A mono sample (backed by its own ArrayBuffer, so Web Audio takes it as is). */
export type Samples = Float32Array<ArrayBuffer>;
/** A partial: [Hz, amplitude, decay seconds, phase?, attack seconds?]. */
export type PartialSpec = readonly [f: number, a: number, tau: number, phase?: number, attack?: number];
/** One note of an instrument at f (Hz), velocity v, made `d` (the seed's make). */
export type Voice = (f: number, v: number, d: Design, rate: number) => Samples;
export type DrumName = "kick" | "snare" | "clap" | "rim" | "hat" | "open" | "shaker";
/** A piece's samples: pitched maps by MIDI root, and the drums by name. */
export interface SampleData<T = Samples> {
  readonly keys: Record<number, T>;
  readonly lead: Record<number, T>;
  readonly bass: Record<number, T>;
  readonly drums: Record<DrumName, T>;
}
export interface Wear { readonly bits?: number; readonly hold?: number; readonly top?: number; readonly dust?: number }
...
function partials(out: Samples, rate: number, list: readonly PartialSpec[], vel = 1): Samples {
  const n = out.length;
...
function onePole<T extends Float32Array>(x: T, hz: number, rate: number, high = false): T {
  const k = Math.exp((-TAU * hz) / rate);
  let y = 0;
  for (let i = 0; i < x.length; i += 1) { y = (1 - k) * x[i]! + k * y; x[i] = high ? x[i]! - y : y; }
  return x;
}
function bandpass<T extends Float32Array>(x: T, f0: number, q: number, rate: number): T {
  const w = (TAU * f0) / rate;
...
const drive = <T extends Float32Array>(x: T, k: number): T => { const n = Math.tanh(k); for (let i = 0; i < x.length; i += 1) x[i] = Math.tanh(x[i]! * k) / n; return x; };
const normalize = <T extends Float32Array>(x: T, peak = 0.9): T => { let m = 1e-9; for (const v of x) m = Math.max(m, Math.abs(v)); const g = peak / m; for (let i = 0; i < x.length; i += 1) x[i]! *= g; return x; };
/** Worn like a sample off a record: fewer bits, held samples (a lower rate), the top off, dust. */
function wear<T extends Float32Array>(x: T, rate: number, { bits = 12, hold = 2, top = 9000, dust = 0 }: Wear, rnd: () => number): T {
  const q = 2 ** (bits - 1);
  let held = 0;
  for (let i = 0; i < x.length; i += 1) {
...
const buf = (rate: number, sec: number): Samples => new Float32Array(Math.round(rate * sec));

// ---------------------------------------------------------------- keys

// Each instrument: one note of it at f (Hz), velocity v, made `d` (the seed's make).
const KEYS: Record<string, Voice> = {
...
function pluck(f: number, v: number, sec: number, bright: number, decay: number, rate: number, seed: number): Samples {
...
const BASS: Record<string, Voice> = {
  ...
  sub: (f, v, _d, rate) => partials(buf(rate, 1.8), rate, [[f, 1, 1.4, 0, 0.012], [f * 2, 0.18, 0.5, 0, 0.012]], v),
  ...
  synth: (f, v, _d, rate) => { const list: PartialSpec[] = []; for (let n = 1; n <= 10; n += 1) list.push([f * n, 0.6 / n, 1.2 / (1 + 0.25 * n), 0, 0.004]); return onePole(partials(buf(rate, 1.6), rate, list, v), 700, rate); },
};

// ---------------------------------------------------------------- drums

type DrumMake = Pick<Design, "seed" | "kickHz" | "kickDecay" | "punch" | "snareBody" | "snareTone" | "snareDecay" | "hatTone">;

const DRUMS = {
  // A kick: a skin whose note drops as it's struck, a click on top, driven.
  kick: (d: DrumMake, rate: number): Samples => {
  ...
  snare: (d: DrumMake, rate: number): Samples => {
  ...
  clap: (d: DrumMake, rate: number): Samples => {
  ...
  rim: (_d: DrumMake, rate: number): Samples => partials(buf(rate, 0.12), rate, [[1720, 1, 0.012, 0, 0.0005], [460, 0.5, 0.02, 0, 0.0005]]),
  // Hats: six square-ish metals at the old machine's odd ratios, high-passed -- closed and open.
  hat: (d: DrumMake, rate: number, open = false): Samples => {
  ...
  shaker: (d: DrumMake, rate: number): Samples => {
...
/** What the samples read off a plan: its players and their make. */
export type SamplePlan = Pick<Plan, "keys" | "lead" | "bass" | "design">;
 ...
 */
export function makeSampleData(plan: SamplePlan, rate = 44100): SampleData {
  const d = plan.design;
  const rnd = noiseOf(d.seed);
  const worn = (x: Samples, extra: Wear = {}) => wear(x, rate, { bits: d.bits, hold: d.hold, top: d.top, dust: 0.0004, ...extra }, rnd);
  const instrument = (make: Voice, lo: number, hi: number, step: number, vel: number) => {
    const map: Record<number, Samples> = {};
    for (let m = lo; m <= hi; m += step) map[m] = normalize(worn(make(midiHz(m), vel, d, rate)), 0.8);
    return map;
  };
  const drums: [DrumName, Samples][] = [
  ...
  return {
    ...
    drums: Object.fromEntries(drums.map(([k, x]) => [k, normalize(worn(x, { top: d.top * 0.8 }), 0.9)])) as Record<DrumName, Samples>,
  };
}

/** A mono AudioBuffer of one Float32Array (a page's AudioBuffer, or `ctx`'s). */
export function toAudioBuffer(x: Samples, rate: number, ctx: BaseAudioContext | null = null): AudioBuffer {
...
export function makeSamples(plan: SamplePlan, rate = 44100): SampleData<AudioBuffer> {
  const data = makeSampleData(plan, rate);
  const map = <K extends PropertyKey>(o: Record<K, Samples>) => Object.fromEntries(Object.entries<Samples>(o).map(([k, x]) => [k, toAudioBuffer(x, rate)])) as Record<K, AudioBuffer>;
