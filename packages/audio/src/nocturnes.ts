...
// for byte (test/equality.test.ts checks tokens 1..300, rooms without a theme
// and every pin).

import { bandNamed, streamOf } from "./score.ts";
import type { BandInput, Mood, Pins, Stream } from "./score.ts";

/** As much of a NOCTURNES genome as the music reads. */
export interface NocturnesGenome {
  readonly seed: string | number;
  readonly setting?: string | undefined;
  readonly canvas?: { readonly theme?: string | undefined; readonly kit?: string | undefined } | undefined;
  readonly monitor?: { readonly crt?: unknown } | undefined;
  readonly placements?: readonly NocturnesPlacement[] | undefined;
  readonly recipe?: { readonly music?: NocturnesMusicPins | undefined } | undefined;
  readonly view?: { readonly kind?: string | undefined; readonly eclipse?: unknown } | undefined;
  readonly palette?: { readonly hue?: number | undefined; readonly scheme?: string | undefined; readonly ramps?: { readonly key?: { readonly hues?: readonly number[] | undefined } | undefined } | undefined } | undefined;
  readonly frames?: number | undefined;
  readonly delay?: number | undefined;
}
export interface NocturnesPlacement {
  readonly obj: { readonly realm?: string; readonly key?: string; readonly kitRole?: string; readonly form?: string; readonly family?: string };
}
/** NOCTURNES' studio pins (recipe.music): "theme" is the band pin. */
export type NocturnesMusicPins = Omit<Pins, "band"> & { readonly theme?: string | undefined };
export interface Item { readonly realm: string | undefined; readonly key: string | undefined }

// A floor room's corner, over its theme.
export const KIT_BAND: Readonly<Record<string, BandInput>> = {
...
const ANCHOR_KIT: Readonly<Record<string, string>> = { tv: "Media Corner", guitar: "Music Corner", telescope: "Stargazing", fan: "Summer Night", speaker: "Teen Room", amp: "Teen Room", chair: "Reading Corner", plant: "Reading Corner", lamp: "Reading Corner" };
// What's out the window, under the record.
export const WEATHER: Readonly<Record<string, readonly string[]>> = {
...
export const DARK: Readonly<Record<string, number>> = { Rain: 0.35, Fog: 0.4, Void: 0.3, Stars: 0.5, Moon: 0.6, Space: 0.55, Aurora: 0.6 };
// Which mode the palette's harmony sounds like.
export const SCHEME_MODE: Readonly<Record<string, string>> = {
...
type RealmKey = readonly [string, string];
// NOCTURNES' themes as far as the music needs them: [name, heroes, company] ([realm, key] each), in its order.
const THEME_ITEMS: readonly (readonly [string, readonly RealmKey[], readonly RealmKey[]])[] = [
...
export function themeForItems(items: readonly Item[], S: Stream): string | null {
...
function themeOfScene(g: NocturnesGenome): string {
...
function kitOfScene(g: NocturnesGenome): string | null {
  ...
  return (anchor && ANCHOR_KIT[anchor.obj.key as string]) ?? g.canvas?.kit ?? null;
}

/** A NOCTURNES genome as a mood (its recipe.music as pins; "theme" is the band pin). */
export function moodOfNocturnes(g: NocturnesGenome): Mood & { readonly seed: string | number; readonly name: string; readonly band: BandInput; readonly pins: Pins } {
  const o: NocturnesMusicPins = g.recipe?.music ?? {};
  const theme = themeOfScene(g);
  const view = g.view?.kind ?? "Stars";
  const has = (re: RegExp) => (g.placements ?? []).some((pl) => re.test(`${pl.obj.form ?? ""} ${pl.obj.family ?? ""}`));
  const room: string[] = [];
  ...
  const kit = kitOfScene(g);
  return {
    seed: g.seed, name: theme,
    band: { ...(bandNamed(theme) ?? {}), ...((kit === null ? undefined : KIT_BAND[kit]) ?? {}) },
    hue: g.palette?.hue ?? g.palette?.ramps?.key?.hues?.[0],
    mode: g.palette?.scheme === undefined ? undefined : SCHEME_MODE[g.palette.scheme], eclipse: Boolean(g.view?.eclipse),
