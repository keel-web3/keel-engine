// The band, on Tone: a plan (./score.ts) played live on Tone's transport, or
...
// samples synthesised from the plan (./samples.ts), the record scratches, the
...
import { makeSamples } from "./samples.ts";
import { MODES, streamOf } from "./score.ts";
import type { Bar, Chord, Plan, Stream } from "./score.ts";
import type { ToneGain, ToneInput, ToneInstrument, ToneLike, ToneNode, ToneParam } from "./tone.ts";
...
export function voice(plan: Pick<Plan, "tonic"> & { readonly design?: { readonly spread?: string } }, chord: Chord, prev: readonly number[] | null): number[] {
...
/** How each layer sits at an intensity. */
export interface IntensityMix {
  ...
  /** The lowpass the band is heard through (Hz). */
  readonly focus: number;
  /** A busier kit on top, 0-1. */
  readonly lift: number;
  /** The tempo's multiplier. */
  readonly rate: number;
}

/** How each layer sits at an intensity (0 idle, 0.5 as composed, 1 driving). Pure numbers: tests read them. */
export function intensityMix(x: unknown, { tempo = 0.04 }: { readonly tempo?: number } = {}): IntensityMix {
  const i = Math.max(0, Math.min(1, Number.isFinite(+(x as number)) ? +(x as number) : 0.5));
  const s = (a: number, b: number) => { const t = Math.max(0, Math.min(1, (i - a) / (b - a))); return t * t * (3 - 2 * t); };
  return {
    intensity: i,
    keys: 1,
...
type Fire<V> = (voice: V, time: number, duration: number) => void;
/** A few of one voice, taken in turn (a single-voice synth can't start a note before its last is done). */
function pool<V>(Tone: ToneLike, make: () => V, n: number, tail: number) {
  const list = Array.from({ length: n }, () => ({ v: make(), free: 0 }));
  return (t: number, dur: number | string, fire: Fire<V>): void => {
...
type DrumHit = "kick" | "snare" | "clap" | "rim" | "hat" | "shaker" | "open";
const DRUM_NOTE: Record<DrumHit, number> = { kick: 24, snare: 26, clap: 27, rim: 28, hat: 30, shaker: 32, open: 34 };

interface Players {
  keys(t: number, notes: readonly number[], dur: number | string, vel: number): void;
  lead(t: number, m: number, dur: number | string, vel: number): void;
  bass(t: number, m: number, dur: number | string, vel: number): void;
  hit(name: DrumHit, t: number, vel: number): void;
}
const KEYS_LEVEL: Record<string, number> = { organ: -15, vibes: -9, guitar: -8, felt: -8 };

/** The players, off this piece's own samples: keys, lead, bass and drums, each through its own chain into `outs`. */
function makePlayers(Tone: ToneLike, plan: Plan, outs: Record<"keys" | "lead" | "bass" | "drums", ToneInput>, cyc: (hz: number) => number): Players {
  ...
  keys.volume.value = KEYS_LEVEL[plan.keys] ?? -10;
  ...
  const urls = Object.fromEntries(Object.entries(DRUM_NOTE).map(([k, n]) => [n, s.drums[k as DrumHit]]));
...
function makeVox(Tone: ToneLike, out: ToneInput) {
  ...
  return (t: number, m: number, dur: number | string, vowel = 0) => voices(t, dur, (v, tt, d) => {
 ...
 */
function makeDeck(Tone: ToneLike, out: ToneInput) {
  ...
  const start = (t: number) => { noise.start(t); osc.start(t); };
  // A stroke: forward (up) or back (down), the level cut in and out by the fader.
  const stroke = (t: number, d: number, up: boolean, hz: number, level: number) => {
    ...
    for (const [gate, l] of [[gateN, level], [gateO, level * 0.35]] as const) {
  ...
  return {
    ...
    scratch: (t: number, hz: number, R: Stream, beatSec: number) => {
...
function makeWeather(Tone: ToneLike, plan: Plan, out: ToneInput, cyc: (hz: number) => number) {
  const layers: { start(t: number): unknown }[] = [];
  const db = (x: number) => 10 ** (x / 20);
  const bed = (type: string, lo: number, hi: number, level: number) => {
  ...
  const swell = (param: ToneParam, rate: number, lo: number, hi: number, lin = false) => new Tone.LFO({ frequency: cyc(rate), min: lin ? lo : db(lo), max: lin ? hi : db(hi) }).start(0).connect(param);
  for (const w of [...plan.weather, ...plan.room]) {
    if (w === "rain") { const r = bed("pink", 400, 3200, -30); swell(r.v.gain, 0.07, -33, -27); }
      ...
      for (const [k, rate] of [[24, 0.05], [31, 0.071], [36, 0.093]] as const) {
  ...
  const events = (t0: number, t1: number, R: Stream) => {
  ...
  return { start: (t: number) => layers.forEach((l) => l.start(t)), events };
}

// ---------------------------------------------------------------- playing it

export interface PlayOptions {
  /** Start the transport from the top now (default true). */
  readonly start?: boolean;
  /** The bar the loop starts at. */
  readonly from?: number;
  /** 0-1 (default: the plan's energy, or 0.5 as composed). */
  readonly intensity?: number;
  /** How far intensity may push the tempo, a fraction each way (0 holds it). */
  readonly tempo?: number;
}
/** A band playing. */
export interface Band {
  /** The reverb's room, worked out before it sounds. */
  readonly ready: Promise<void>;
  /** Out over a moment, then everything it made is gone (the transport too, unless another band follows). */
  stop(fade?: number, opts?: { readonly transport?: boolean }): void;
...
type Section = Bar & { readonly index: number };
 ...
 */
export function play(Tone0: ToneLike, plan: Plan, out: ToneInput | null = null, { start = true, from = 0, intensity = plan.energy ?? 0.5, tempo = 0.04 }: PlayOptions = {}): Band {
  // (Everything made here is kept, so stopping takes all of it away.)
  const made: { dispose(): unknown }[] = [];
  const Tone = new Proxy(Tone0, {
    get(t, k) {
      const v: unknown = Reflect.get(t, k);
      if (typeof v !== "function" || typeof k !== "string" || !/^[A-Z]/.test(k) || !(v.prototype as { dispose?: unknown } | undefined)?.dispose) return v;
      return new Proxy(v, { construct(C, args) { const o = new (C as new (...a: unknown[]) => { dispose(): unknown })(...args); made.push(o); return o; } });
    },
  });
  ...
  const cyc = (hz: number) => Math.max(1, Math.round(hz * plan.loopSec)) / plan.loopSec;
  ...
  wow.connect(focus);
  const level = (x: number, to: ToneNode): ToneGain => new Tone.Gain(x).connect(to);
  ...
  const pad: ToneInstrument = new Tone.PolySynth(Tone.Synth, { oscillator: { type: "fatsawtooth", count: 3, spread: 16 }, envelope: { attack: 1.2, decay: 1, sustain: 0.7, release: 2.5 } });
  pad.volume.value = -28;
  ...
  const barAt = (i: number): Bar => plan.bars[i]!;

  // The lead's line for a bar: the motif, developed through the section.
  function melody(bar: Section): [number, number, number][] {
    const m = plan.motif;
    ...
    const startOn = (chord: Chord) => { const cand = chord.tones.map((x) => home - 5 + ((x - (home - 5 - plan.tonic)) % 12 + 12) % 12); return cand.reduce((a, b) => (Math.abs(b - home) < Math.abs(a - home) ? b : a)); };
    const half = bar.k % 2;
    const phraseFirst = barAt((bar.index - half + L) % L);
    let moves = m.moves;
    let shift = 0;
  ...
  const bar = (time: number, n: number) => {
    ...
    const at = (step: number) => Math.max(time, time + step * sixteenth + R.between(-0.006, 0.012)); // (a hand's looseness, never before the bar)
    const sec = b.sec;
    const last = b.k === b.of - 1;
    const gr = plan.groove;

    // ---- keys: this seed's way of comping, busier in B, held in the break
    const chordAt = (t: number, dur: number | string, vel: number) => {
    ...
    if (sec === "C") chordAt(at(0), "1m", 0.5);
      ...
      type Hit = readonly [number, string, number];
      const hitsOf = (): readonly Hit[] | undefined => {
        // (The stabs draw whatever the comp -- the whole table is built, as in the proof of concept -- so the bar's stream stays in step.)
        const stabs = sec === "B" ? R.pick<readonly Hit[]>([[[0, "4n.", 0.62], [6, "8n", 0.4], [10, "4n", 0.5]], [[0, "8n", 0.6], [3, "8n", 0.4], [8, "4n.", 0.55]]]) : R.pick<readonly Hit[]>([[[0, "2n", 0.6], [10, "4n", 0.45]], [[0, "4n.", 0.6], [11, "8n", 0.4]]]);
        const table: Record<string, readonly Hit[]> = {
          sustain: sec === "B" ? [[0, "2n", 0.6], [8, "2n", 0.45]] : [[0, "1m", 0.6]],
          push: [[0, "8n", 0.6], [7, "4n", 0.5], [14, "8n", 0.42]],
          strum: [[0, "4n.", 0.62], [6, "8n", 0.38], [8, "4n", 0.55], [14, "8n", 0.35]],
          stabs,
        };
        return table[plan.comp];
      };
      const hits = hitsOf() ?? [[0, "2n", 0.6]];
    ...
    if (sec === "C") { if (b.k === 0) band.bass(at(0), root, "1m", 0.7); }
      ...
      const fifth = root + 7;
      type Note = readonly [number, number, string];
      const line: Record<string, readonly Note[]> = {
      ...
      for (const [s, m, dur] of line[plan.bassStyle]!) if (s === 0 || R.chance(sec === "A" ? 0.75 : 0.9)) band.bass(at(s), m, dur, s === 0 ? 0.9 : 0.7);
    }
  ...
  return {
    ...
    stop: (fade = 0.4, { transport = true } = {}) => {
      ...
      setTimeout(() => { for (const o of made) { try { o.dispose(); } catch { /* (already gone) */ } } }, fade * 1000 + 80);
    },
    ...
    setIntensity(x, ramp = 1.2) {
      const nextMix = intensityMix(x, { tempo });
      if (Math.abs(nextMix.intensity - mix.intensity) < 0.01) return; // (called every step: only a real change moves anything)
      mix = nextMix;
      for (const k of ["keys", "lead", "bass", "drums"] as const) layer[k].gain.rampTo(mix[k], ramp);
...
export interface RenderOptions {
  /** Bars played before the top, their tails ringing into it (default 4). */
  readonly preroll?: number;
  readonly intensity?: number;
  readonly rate?: number;
  /** Seconds more past the loop's end: what the band plays going round. */
 ...
 */
export async function render(Tone: ToneLike, plan: Plan, { preroll = 4, intensity, rate = 44100, overrun = 0 }: RenderOptions = {}): Promise<AudioBuffer> {
  ...
  const raw = ("get" in buf ? buf.get() : undefined) ?? (buf as AudioBuffer);
