...
// (The proof of concept's tests/audio.test.mjs, on the port.)
...
const combo = (p: Plan) => [p.keys, p.lead, p.bass, p.kit].join("|");
const WALLRUN: MoodSpec = { name: "Night Water", energy: 0.65, darkness: 0.7, weather: ["waves", "wind"] };

test("manifest: keel/audio, a runtime part that needs only the codec", () => {
  assert.equal(manifest.id, "keel/audio");
  assert.equal(manifest.version, "0.1.0");
  assert.equal(manifest.kind, "runtime");
  assert.deepEqual(manifest.needs, ["keel/codec@^0.1"]);
  assert.equal(manifest.schema, "keel-engine-module@1");
});
...
test("diversity: NOCTURNES tokens 1..200 through the engine, and games from their seeds", { skip: !hasNocturnes && `NOCTURNES isn't at ${NOCTURNES}` }, () => {
  ...
  for (const [name, spec] of Object.entries({ wallrun: WALLRUN, idle: { energy: 0.1, darkness: 0.3 }, driving: { energy: 1, darkness: 0.9, weather: ["rain"] } })) {
    const mood = moodFor(spec);
    const t = tally(Array.from({ length: 200 }, (_, i) => scoreOf(mood, `${name} ${i}`)));
...
test("moods: energy and darkness move the band the way they say", () => {
  const avg = (spec: MoodSpec, f: (p: Plan) => number) => { const m = moodFor(spec); let s = 0; for (let i = 0; i < 150; i += 1) s += f(scoreOf(m, `m${i}`)); return s / 150; };
  ...
  for (const x of [-1, 0, 0.5, 1, 2, NaN, "x"]) finite(moodFor({ energy: x as number, darkness: x as number }), `moodFor(${x})`);
});
...
test("intensity: layers in and out by plain numbers; 0.5 is the plan as composed", () => {
  ...
  let prev: IntensityMix | null = null;
  const layers = ["keys", "drums", "bass", "lead", "focus", "lift", "rate"] as const;
  for (let x = -0.5; x <= 1.5; x += 0.01) {
    ...
    for (const k of ["keys", "drums", "bass", "lead", "lift"] as const) assert.ok(m[k] >= 0 && m[k] <= 1);
    prev = m;
  }
...
test("instruments: every sample sounds, never clips, no NaN; different seeds make different instruments", () => {
  ...
  for (const [i, spec] of [WALLRUN, { energy: 0.1 }, { energy: 1, darkness: 1 }].entries()) {
    ...
    for (const part of ["keys", "lead", "bass", "drums"] as const) {
      for (const [k, x] of Object.entries(s[part] as Record<string, Float32Array>)) {
