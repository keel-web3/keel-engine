...
// (The proof of concept's tests/audio-sfx.test.mjs, on the port. The body is
// the proof of concept's own character controller, imported as JS until
...
test("sounds: every one finite, heard, under full scale; the loops seamless", () => {
  for (const [seed, style] of [["wallrun", "lofi"], ["7", "chip"], ["b", "soft"], ["c", "clean"]] as const) {
    ...
    const oneShots = ["jump", "land", "land_hard", "wallStart", "wallJump", "railStart", "railEnd", "splash", "skimStart", "respawn", "blip", "hover", "select", "back", "confirm", "error"] as const;
    for (const k of [...SURFACES.map((s) => `step_${s}` as const), ...oneShots]) {
      for (const x of d[k]) {
...
test("sounds: deterministic per seed, different between seeds", () => {
  ...
  for (const k of Object.keys(a) as SfxSampleKey[]) {
    const [x, y, z] = [a[k], b[k], c[k]].map((v) => (Array.isArray(v) ? v[0]! : v)) as [Float32Array, Float32Array, Float32Array];
...
test("params: every sound's rate, gain, pan and cutoff stay in range for any input", () => {
  const st = sfxStyle("p");
  const inputs: SfxParams[] = [{}, { speed: -5 }, { speed: 0 }, { speed: 3 }, { speed: 9.5 }, { speed: 14 }, { speed: 40 }, { speed: 1e9 }, { speed: NaN }, { gain: 3, pan: -9 }, { gain: -1, pan: 9 }, { rate: 100 }, { rate: 0 }, { surface: "metal" }, { surface: "lava" }];
  for (const name of [...SFX_NAMES, ...LOOP_NAMES, "nope"]) {
    for (const p of inputs) {
      const q = paramsFor(name, p, st);
      for (const k of ["rate", "gain", "pan", "cutoff"] as const) assert.ok(Number.isFinite(q[k]), `${name} ${k} ${JSON.stringify(p)}`);
...
test("bodySfx: a real body (the proof of concept's character) runs, jumps, lands, wall-runs, skims and sinks", { skip: !hasPoc && `the proof of concept isn't at ${POC}` }, () => {
  ...
  const dt = 1 / 120;
  const run = (sec: number, input: (t: number) => { move: [number, number]; jump?: boolean; hold?: boolean }) => { for (let i = 0; i < sec / dt; i += 1) { body.step(dt, input(i * dt)); for (const e of body.events) seen.add(e.type); feet.update(body, dt); } };
  ...
  assert.ok(steps.some(([, p]) => p.surface === "stone") && steps.every(([, p]) => SURFACES.includes(p.surface as Surface)));
  assert.ok(names.includes("jump") && names.includes("land"), `jump and land (${[...new Set(names)].join(", ")})`);
  ...
  const modeOf: Record<string, string> = { grind: "grind", wallrun: "wall", skim: "skim" };
  for (const k of ["grind", "wallrun", "skim"]) for (const h of rec.loops[k] ?? []) assert.ok(h.stopped || modeOf[k] === body.mode, `${k} let go`);
  feet.stop();
...
test("bodySfx: each event, scripted", () => {
  const rec = recorder();
  const feet = bodySfx(rec.player, { wind: false });
  const body = (mode: string, events: { type: string; speed?: number }[], vel = [0, 0, 8]): SfxBody => ({ mode, events, vel });
