...
// and a wall run tilted the right way. (The proof of concept's
// tests/entity-clips.test.mjs, ported.)
import { test } from "node:test";
import assert from "node:assert/strict";
import { frontOf, rightOf, worldToLocal } from "@keel-engine/core";
import type { Vec3, Vec3Like } from "@keel-engine/core";
import { GAITS, KINDS, LAND_TIME, animator, entityOf, featurePoints, posed, skinOf } from "../src/index.ts";
import type { Animator, BodyMode, Capsule, EntitySpec, Kind, Species } from "../src/index.ts";
...
const sole = (c: Pick<Capsule, "a" | "b" | "r">): number => Math.min(c.a[1], c.b[1]) - c.r;
const reachOf = (spec: EntitySpec): number => (spec.plan === "quadruped" ? spec.body.shoulderH - spec.body.ankleH : spec.body.hipH - spec.body.ankleH);
const pt = (F: Record<string, Vec3>, k: string): Vec3 => { const p = F[k]; assert.ok(p, `feature ${k}`); return p; };
const partOf = (caps: readonly Capsule[], part: string): Capsule => { const c = caps.find((q) => q.part === part); assert.ok(c, `part ${part}`); return c; };

/** When each foot touches down over one cycle (phases 0..1), read off the posed paws. */
function touchdowns(spec: EntitySpec, clip: string, feet: readonly string[], N = 400): Record<string, number> {
  const down = (i: number): Record<string, boolean> => {
    const caps = skinOf(spec, posed(spec, clip, { phase: i / N, yaw: 0.8 }));
    return Object.fromEntries(feet.map((k) => [k, sole(partOf(caps, k)) < 1e-9 * spec.body.H]));
  };
...
const cyclicOrder = (td: Record<string, number>, first: string): string[] => Object.keys(td).sort((a, b) => ((td[a]! - td[first]! + 1) % 1) - ((td[b]! - td[first]! + 1) % 1));
...
test("quadruped footfalls: walk HL FL HR FR, trot in diagonal pairs, gallop hinds then fores, bound in pairs", () => {
  ...
  for (const sp of ["cat", "dog", "fox", "bear", "deer"] as const) {
  ...
  for (const sp of ["rabbit", "mouse"] as const satisfies readonly Species[]) {
...
test("a biped walk always has a foot down; a fast run has both feet up for a moment; arms swing against legs", () => {
  for (const kind of ["humanoid", "anthro"] as const) {
    for (const seed of ["1", "2", "3", "4", "5"]) {
      ...
      for (let i = 0; i < 100; i += 1) {
        ...
        const local = (p: Vec3Like): Vec3 => worldToLocal(pos, yaw, p);
        const legL = local(pt(F, "toe.L"))[2] - local(pt(F, "toe.R"))[2];
        const armL = local(pt(F, "hand.L"))[2] - local(pt(F, "hand.R"))[2];
...
interface Frame { t: number; caps: Capsule[]; clip: string | null }
/** A body running straight along a heading at a speed, stepped at 120 Hz. */
function runBody(anim: Animator, { yaw, speed, seconds, mode = "ground", wall, from = [2, 0, -1] }: { yaw: number; speed: number; seconds: number; mode?: BodyMode; wall?: Vec3; from?: Vec3 }): Frame[] {
  const f = frontOf(yaw);
  const body = { pos: [...from] as Vec3, vel: [f[0] * speed, 0, f[2] * speed] as Vec3, facing: yaw, mode, wall };
  const frames: Frame[] = [];
  const dt = 1 / 120;
...
test("planted feet stay planted: the animator's phase runs on distance, so nothing slides", () => {
  const cases: Array<[Kind, number, string]> = [
      ...
      for (const yaw of [0.4, -2.3]) {
        ...
        for (let i = 180; i < frames.length; i += 1) {
          for (const c of frames[i]!.caps) {
            ...
            const before = partOf(frames[i - 1]!.caps, c.part);
...
const jointsOf = (anim: Animator): Record<string, Vec3> => Object.fromEntries(Object.entries(anim.skeleton().bones).map(([n, b]) => [n, b.p]));

/** The most any joint moves in a frame (less the body's own move) in steady gaits from a stroll to a sprint. */
function steadyMotion(spec: EntitySpec): number {
  const reach = reachOf(spec);
  let most = 0;
  for (let k = 1; k <= 10; k += 1) {
    const anim = animator(spec);
    const body = { pos: [0, 0, 0] as Vec3, vel: [0, 0, reach * k] as Vec3, facing: 0, mode: "ground" as const };
    let prev: Record<string, Vec3> | null = null;
    for (let i = 0; i < 200; i += 1) {
      body.pos = [0, 0, body.pos[2] + (reach * k) / 60];
      anim.step(1 / 60, body);
      const now = jointsOf(anim);
      if (prev && i > 120) for (const [n, p] of Object.entries(now)) most = Math.max(most, Math.hypot(...sub(p, prev[n]!)) - (reach * k) / 60);
      prev = now;
    }
  }
  return most;
...
test("the animator follows the body's mode, crossfades without pops, and is deterministic", () => {
  for (const kind of KINDS) {
    ...
    const run = (anim: Animator) => {
      const body = { pos: [0, 0, 0] as Vec3, vel: [0, 0, 0] as Vec3, facing: 0.5, mode: "ground" as BodyMode };
      const seen: Array<string | null> = [];
      const dt = 1 / 60;
      const script = (t: number): { v: number; mode: BodyMode; vy: number } => {
      ...
      for (let t = 0; t < 4; t += dt) {
        ...
        if (prev) {
          ...
          for (const [name, p] of Object.entries(now)) pop = Math.max(pop, Math.hypot(...sub(p, prev[name]!)) - move);
        }
        prev = now;
      }
...
test("phase is continuous through a walk-to-run blend", () => {
  ...
  const reach = reachOf(spec);
  const anim = animator(spec);
  const body = { pos: [0, 0, 0] as Vec3, vel: [0, 0, 0] as Vec3, facing: 0, mode: "ground" as const };
...
test("on a wall: feet toward it, head away, whichever side it's on; wallGap puts the feet on it", () => {
  for (const kind of ["anthro", "humanoid"] as const) {
    for (const seed of ["1", "6"]) {
      const spec = entityOf(seed, { kind });
      assert.ok(spec.plan === "humanoid");
      for (const yaw of [0.2, 2.9, -1.4]) {
        for (const side of [1, -1]) {
          ...
          const n = rightOf(yaw).map((v) => v * side) as Vec3;
          const anim = animator(spec, { fade: 0 });
          const f = frontOf(yaw);
          const body = { pos: [0, 1, 0] as Vec3, vel: [f[0] * 8, 1, f[2] * 8] as Vec3, facing: yaw, mode: "wall" as const, wall: n };
          anim.step(1 / 60, body);
          const F = featurePoints(anim.capsules());
          const away = dot(sub(pt(F, "head"), [0, 1, 0]), n);
          assert.ok(away > spec.body.H * 0.1, `${kind} ${seed} yaw ${yaw} wall ${side > 0 ? "left" : "right"}: head leans off the wall (${away.toFixed(3)})`);
          const feet: Vec3 = [(pt(F, "toe.L")[0] + pt(F, "toe.R")[0]) / 2, 0, (pt(F, "toe.L")[2] + pt(F, "toe.R")[2]) / 2];
          assert.ok(dot(sub(feet, pt(F, "head")), n) < 0, "feet nearer the wall than the head");
          ...
          const moved = dot(sub(pt(G, "toe.L"), pt(F, "toe.L")), n);
...
test("hold and release a clip; a landing squashes then recovers", () => {
  ...
  const hip = (landT: number): number => posed(p, "land", { landT }).bones["hips"]!.p[1];
