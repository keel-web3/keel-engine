...
// are where the core frame says they are -- at every yaw, for every kind.
// (The proof of concept's tests/entity.test.mjs, ported.)
import { test } from "node:test";
import assert from "node:assert/strict";
import { frontOf, rightOf, yawOf } from "@keel-engine/core";
...
import {
  ...
  lowestY, materialFor, measuredLengths, optionsOf, posed, readsOf, restLengths, rotX, rotY, rotZ, skinOf, solveTwoBone,
} from "../src/index.ts";
import type { Capsule, ChoiceDef, ChoiceValues, EntityPins, EntitySpec, Kind } from "../src/index.ts";
...
const near = (a: Vec3Like, b: Vec3Like, e: number): boolean => a.every((v, i) => Math.abs(v - b[i]!) < e);
const finite = (caps: readonly Capsule[]): boolean => caps.every((c) => [...c.a, ...c.b, c.r].every(Number.isFinite) && c.r > 0 && Number.isInteger(c.mat));
const pt = (F: Record<string, Vec3>, k: string): Vec3 => { const p = F[k]; assert.ok(p, `feature ${k}`); return p; };
const specs = {} as Record<Kind, EntitySpec[]>;
for (const kind of KINDS) specs[kind] = SEEDS.map((s) => entityOf(s, { kind }));

test("rotations turn the way the core frame does, and the pose signs are as documented", () => {
  for (const y of YAWS) {
...
test("two-bone IK keeps both lengths, reaches what it can, and bends toward the pole", () => {
  for (let i = 0; i < 400; i += 1) {
    ...
    const a: Vec3 = [Math.sin(i) * 2, 1 + Math.cos(i * 1.3), Math.sin(i * 0.7)];
    const t: Vec3 = [a[0] + Math.sin(i * 2.1) * 0.9, a[1] - Math.abs(Math.cos(i * 0.3)) * 0.9, a[2] + Math.cos(i * 1.7) * 0.9];
    const pole: Vec3 = [Math.sin(i * 0.9), 0.2, Math.cos(i * 0.9)];
    ...
    const along = dot(sub(mid, a), nn);
    const off = sub(sub(mid, a), [nn[0] * along, nn[1] * along, nn[2] * along]);
  ...
  for (const kind of KINDS) {
    for (const spec of specs[kind]) {
      for (const yaw of YAWS) {
        ...
        for (const k of ["eye.L", "eye.R", "nose", "face"]) assert.ok(dot(sub(pt(F, k), pt(F, "centre")), f) > 0, `${k} in front of the body: ${where}`);
        for (const k of ["eye.L", "eye.R", "nose"]) assert.ok(dot(sub(pt(F, k), pt(F, "head")), f) > 0, `${k} on the front of the head: ${where}`);
        const toes = Object.keys(F).filter((k) => k.startsWith("toe."));
        assert.equal(toes.length, spec.plan === "quadruped" ? 4 : 2);
        const tc = toes.reduce<Vec3>((s, k) => [s[0] + pt(F, k)[0] / toes.length, s[1] + pt(F, k)[1] / toes.length, s[2] + pt(F, k)[2] / toes.length], [0, 0, 0]);
        assert.ok(dot(sub(tc, pt(F, "centre")), f) > 0, `toes in front of the body: ${where}`);
        for (const k of toes) assert.ok(dot(sub(pt(F, k), pt(F, `heel.${k.slice(4)}`)), f) > 0, `${k} ahead of its heel: ${where}`);
        assert.ok(dot(sub(pt(F, "eye.R"), pt(F, "eye.L")), r) > 0, `right eye on the right: ${where}`);
        if (spec.plan === "humanoid") {
          assert.ok(dot(sub(pt(F, "hand.R"), pt(F, "centre")), r) > 0, `right hand on the right: ${where}`);
          assert.ok(dot(sub(pt(F, "hand.L"), pt(F, "centre")), r) < 0, `left hand on the left: ${where}`);
          assert.ok(dot(sub(pt(F, "toe.R"), pt(F, "toe.L")), r) > 0, `right foot on the right: ${where}`);
        } else {
          assert.ok(dot(sub(pt(F, "toe.FR"), pt(F, "toe.FL")), r) > 0 && dot(sub(pt(F, "toe.HR"), pt(F, "toe.HL")), r) > 0, `right paws on the right: ${where}`);
          assert.ok(dot(sub(pt(F, "toe.FL"), pt(F, "toe.HL")), f) > 0, `fore paws ahead of the hinds: ${where}`);
        }
    ...
    for (const spec of specs[kind].slice(0, 60)) {
      ...
      for (const clip of Object.keys(clipsFor(spec))) {
        for (const [i, yaw] of YAWS.filter((_, j) => j % 4 === 1).entries()) {
          ...
          const F = featurePoints(caps);
          const f = frontOf(yaw);
          assert.ok(dot(sub(pt(F, "face"), pt(F, "head")), f) > 0, `face on the front of the head: ${where}`);
          for (const k of Object.keys(F).filter((q) => q.startsWith("toe."))) assert.ok(dot(sub(pt(F, k), pt(F, `heel.${k.slice(4)}`)), f) > 0, `${k} ahead of its heel: ${where}`);
          if (spec.plan === "humanoid") assert.ok(dot(sub(pt(F, "hand.R"), pt(F, "hand.L")), rightOf(yaw)) > 0, `right hand right of the left: ${where}`);
          const seen = frontOfEntity(caps, { yaw });
...
test("pinning a choice moves nothing that doesn't read it", () => {
  const other = (ch: ChoiceDef, c: ChoiceValues): unknown => {
    const opts = optionsOf(ch.name, c) as readonly unknown[] | undefined;
    const now = c[ch.name];
    if (opts) return opts.find((o) => o !== now);
    if (typeof now === "number") return now * 0.97;
    return undefined;
  };
  for (const kind of KINDS) {
    for (const seed of SEEDS.slice(0, 40)) {
      ...
      for (const ch of CHOICES) {
        ...
        const v = other(ch as ChoiceDef, base.choices);
        if (v === undefined) continue;
        const pinned = entityOf(seed, { kind, pins: { [ch.name]: v } as EntityPins });
  ...
  assert.throws(() => entityOf("1", { pins: { wings: 2 } as EntityPins }), TypeError);
});
...
test("every species of every kind can be made, and a size pin sets the size", () => {
  ...
  const small = entityOf("3", { kind: "animal", size: 0.4 });
  assert.equal(small.plan === "quadruped" && small.body.shoulderH, 0.4);
});
...
test("frontOfEntity: declared, from a skeleton, and a wrongly turned model is caught", () => {
  ...
  const c = pt(featurePoints(caps), "centre");
  const turn = (p: Vec3Like): Vec3 => [2 * c[0] - p[0], p[1], 2 * c[2] - p[2]];
  ...
  const f = frontOf(2);
  const r = rightOf(2);
  const mirror = (p: Vec3Like): Vec3 => { const d = sub(p, c); const k = 2 * dot(d, r); return [p[0] - r[0] * k, p[1], p[2] - r[2] * k]; };
