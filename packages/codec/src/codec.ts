// The codec: a schema compiled once into writer/reader functions, then any
// number of values through them. Canonical -- a value has one encoding (maps
// sorted, runs maximal, the smallest number kind, zero padding, trailing
// empty extension groups left off) -- so equal values are equal bytes and
// bytes can be hashed. Every error says where (the field path) and why:
//
//   parts[3].shape(box).h[1]: 2000.5 is above fixed(0, 1024, 0.001)'s max 1024.
//
// This file is the raw body (no header): document.ts adds the schema id or
// the schema itself in front.
...
const fail = (why: string): never => { throw new CodecError(why); };
...
const fresh = (): Ctx => ({ tables: new Tables(), trace: null, strict: true, text: new BitWriter(64), tr: null, last: null });
const lastOf = (x: Ctx): Map<object, number> => (x.last ??= new Map());
...
function wr(c: C, o: BitWriter, v: unknown, x: Ctx, label: string | number): void {
  try { c.w(o, v, x); } catch (e) { throw under(e, label, null); }
}
function rd(c: C, i: BitReader, x: Ctx, label: string | number): unknown {
  const start = i.at;
  try {
...
const meta = (x: Ctx, label: string, start: number, i: BitReader, value: unknown, note = ""): void => { x.trace?.meta(label, start, i.at - start, value, note); };
...
const unzig = (u: number): number => (u % 2 ? -(u + 1) / 2 : u / 2);
const describe = (v: unknown): string => (typeof v === "string" ? JSON.stringify(v.length > 40 ? `${v.slice(0, 40)}...` : v) : typeof v === "bigint" ? `${v}n` : Array.isArray(v) ? `an array of ${v.length}` : v instanceof Uint8Array ? `${v.length} bytes` : v === null ? "null" : typeof v === "object" ? "an object" : String(v));
...
function intCheck(v: unknown, what: string): number {
...
function writeNum(o: BitWriter, v: number, tagBits: number): void {
...
function readNum(i: BitReader, tag: number): number {
...
function fromUtf8(tr: BitReader, len: number): string {
...
const SCRATCH = new BitWriter(64);
