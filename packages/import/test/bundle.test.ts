// The KEEL build: keel/import bundles into one module that reaches only what
// its manifest needs (core, runtime, entity, object, builder), starts on a
// page (a VM standing in for KEEL's frame) with just its closure loaded, and
// imports there -- a GLB dog read, voxelised, split, its collar on its neck.
...
test(`${ID} bundles to a module that reaches only what it needs, and imports on a page`, async () => {
  ...
  const dog = api["dogSample"]!() as { bytes: Uint8Array };
  const res = api["importModel"]!(dog.bytes, { name: "dog", voxels: 40 }) as { proposal: { kind: string; attributes: Array<{ part: string; slot: string }> } };
  assert.equal(res.proposal.kind, "creature");
  // (Arrays from the page's realm: compared as JSON.)
  assert.equal(JSON.stringify(res.proposal.attributes.map((a) => [a.part, a.slot])), JSON.stringify([["collar", "neck"]]));
});
