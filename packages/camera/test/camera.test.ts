// The port of the proof of concept's tests/camera.test.mjs: the orbit the
// player turns, the chase the attract mode rides, first person, the showcase
// frame, rails -- all on the one frame convention, all stepped, all repeatable.
// (It drives the real body and the real input, as the original did.)

import { test } from "node:test";
import assert from "node:assert/strict";
import { cameraBasis, frontOf, localToWorld, rightOf, wrapAngle } from "@keel-engine/core";
...
// (Test-only, by path: input is not one of camera's dependencies -- see README.)
import { createInput } from "@keel-engine/input";
import { clearance, createCamera, fovForTarget, frameView, railRig, sphereCast, subjectOf } from "../src/camera.ts";
import type { Camera, CameraWorld, LookInput, Subject, View } from "../src/camera.ts";
...
const flat = (v: Vec3Like): Vec3 => { const l = Math.hypot(v[0], v[2]) || 1; return [v[0] / l, 0, v[2] / l]; };
const YAWS = Array.from({ length: 16 }, (_, i) => -Math.PI + (i + 0.5) * (Math.PI / 8));
const FLOOR: Box = { c: [0, -1, 0], h: [40, 1, 40], mat: 1 };
type MutableSubject = { -readonly [K in keyof Subject]: Subject[K] };
const subjectAt = (pos: Vec3, yaw = 0, extra: Partial<Subject> = {}): MutableSubject => ({ pos, yaw, vel: [0, 0, 0], height: 1.1, radius: 0.26, ...extra });
const run = (cam: Camera, s: Subject, world: CameraWorld, n: number, input: LookInput = {}): Camera => { for (let i = 0; i < n; i += 1) cam.step(STEP, s, world, input); return cam; };
// Where a world point lands on the picture (-1..1 each way), the renderer's own projection.
function screenOf(view: View, p: Vec3Like, aspect = 1): [number, number, number] {
...
test("orbit: W runs along the camera's yaw (and D to the screen's right) at 16 yaws -- through the real body", () => {
  for (const y of YAWS) {
    ...
    for (const [key, want] of [["w", seen], ["d", right], ["s", neg(seen)], ["a", neg(right)]] as const) {
...
test("orbit: mouse up looks up; pitch clamps both ways", () => {
  ...
  assert.ok(Math.abs(cam.pitch - 0.5) < 1e-9 && Math.abs(cam.rigs.orbit.pitch - 0.5) < 1e-12);
  run(cam, s, world, 10, { look: [0, -1] });
  assert.ok(Math.abs(cam.pitch + 1.1) < 1e-9 && Math.abs(cam.rigs.orbit.pitch + 1.1) < 1e-12);
...
test("sphereCast: a point it returns always has the radius of room", () => {
  const world: CameraWorld = { boxes: [FLOOR, { c: [0, 2, 3], h: [2, 2, 0.25], yaw: 0.4 }] };
  for (const y of YAWS) {
    const from: Vec3 = [0, 1, 0];
    const to: Vec3 = [Math.sin(y) * 6, 1.5, Math.cos(y) * 6];
    const t = sphereCast(world, from, to, 0.2);
    const l = Math.hypot(...sub(to, from));
    const p = from.map((v, i) => v + ((to[i]! - v) * t) / l) as Vec3;
...
test("orbit: a wall behind pulls the eye in -- never inside a box -- and it eases back out when the wall goes", () => {
  const wall: Box = { c: [0, 2, -1.2], h: [4, 3, 0.3], mat: 0 }; // (just behind a subject facing +z)
  const roof: Box = { c: [5, 1.9, 0], h: [1.5, 0.3, 3], mat: 0 }; // (a low tunnel roof over x = 5)
  const post: Box = { c: [1.4, 1, 1.6], h: [0.3, 1.5, 0.3], yaw: 0.7, mat: 0 };
  const world = { boxes: [FLOOR, wall, roof, post] };
  const R = 0.2;
  for (const [x, z] of [[0, 0], [5, 0], [1.2, 0.8]] as const) {
    const cam = createCamera({ mode: "orbit" });
  ...
  const arm = (): number => cam.rigs.orbit.arm!;
  assert.ok(arm() < 2, `pulled in (${arm().toFixed(2)})`);
  let last = arm();
  for (let i = 0; i < 360; i += 1) {
    cam.step(STEP, s, { boxes: [FLOOR] }, {});
    assert.ok(arm() >= last - 1e-12, "it never snaps in without a reason");
    assert.ok(arm() - last < 0.06, "and eases out rather than jumping");
    last = arm();
  }
  assert.ok(Math.abs(arm() - cam.rigs.orbit.opt.distance) < 0.02, `back out to the full arm (${arm().toFixed(3)})`);
});

test("orbit and chase in a low tunnel: the arm slides back under the roof instead of folding into the head", () => {
  const roof: Box = { c: [0, 2.3, 0], h: [2.6, 0.25, 6], mat: 0 }; // (its underside at 2.05)
  const walls: Box[] = [{ c: [-2.35, 1, 0], h: [0.25, 1.1, 6] }, { c: [2.35, 1, 0], h: [0.25, 1.1, 6] }];
  const world = { boxes: [FLOOR, roof, ...walls] };
  for (const mode of ["orbit", "chase"] as const) {
...
test("hidesSubject: first person, or an eye pulled right into the subject's face", () => {
  const cam = createCamera({ mode: "orbit" });
  const s = subjectAt([0, 0, 0], 0);
  const tight = { boxes: [FLOOR, { c: [0, 1.5, -0.55], h: [3, 3, 0.2] }, { c: [0, 1.6, 0], h: [3, 0.3, 3] }] as Box[] }; // (a wall at its back, a roof just over its head)
  run(cam, s, tight, 10);
  ...
  for (const y of YAWS) {
    for (const turn of [0, 0.45, -0.7]) {
      for (const aspect of [1, 16 / 9, 0.5]) {
        const bounds = [2.6, 0.2, -2.3, 3.4, 1.9, -1.7] as const;
        const s = subjectAt([3, 0.2, -2], y, { bounds });
...
test("frame: seen from the front, the subject's right hand is on the screen's left (a mirror, as in life)", () => {
  ...
  for (const y of YAWS) {
    const s = subjectAt([0, 0, 0], y);
    cam.rigs.frame.enter!(cam, s, true);
    cam.step(STEP, s, {});
...
test("frame: in a level, a wall in front of the subject brings the shot in (never inside it)", () => {
  const wall: Box = { c: [0, 1.5, 1.2], h: [4, 2, 0.25] }; // (a wall 1.2 in front of a subject facing +z)
...
test("rail: passes through its keys at their times, loops, looks at the subject when a key has no target", () => {
  const keys = [{ at: 0, eye: [0, 2, -5] as Vec3 }, { at: 2, eye: [5, 3, 0] as Vec3, target: [0, 0, 0] as Vec3 }, { at: 4, eye: [0, 2, 5] as Vec3 }];
  const r = railRig({ keys });
  const s = subjectAt([1, 0, 1]);
  const close = (a: readonly number[], b: readonly number[]): boolean => a.every((v, i) => Math.abs(v - b[i]!) < 1e-9);
...
test("determinism: the same steps give the same camera, bit for bit, through every mode", () => {
  const once = (): number[] => {
    ...
    const boxes: Box[] = [FLOOR, { c: [3, 2, 6], h: [0.3, 3, 4] }];
    const body = createCharacter({ boxes, rails: [], waterY: -10, spawn: [0, 0.1, 0] });
    const world = { boxes };
    const trace: number[] = [];
    for (let i = 0; i < 1200; i += 1) {
      ...
      const look = [Math.sin(i * 0.013) * 0.01, Math.cos(i * 0.021) * 0.004] as const;
