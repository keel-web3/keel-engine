// The port of the proof of concept's tests/front.test.mjs: fronts found from
// what things are made of, at any yaw.

import { test } from "node:test";
import assert from "node:assert/strict";
import { localToWorld, wrapAngle } from "@keel-engine/core";
import { createEntity } from "../src/entity.ts";
import { box, capsule, lathePart, mk, sphere, torus } from "../src/kit.ts";
import type { SdfPart } from "../src/kit.ts";
import { toPart } from "../src/prims.ts";
import type { BoxSpec, CapsuleSpec, PartExtras, PartLike } from "../src/prims.ts";
import { FEATURES, angleBetween, defineFeature, detectFront, featureOf, frontEvidence, parseFront } from "../src/front.ts";
...
type Raw = (PartExtras & BoxSpec) | (PartExtras & CapsuleSpec);
const isBox = (p: Raw): p is PartExtras & BoxSpec => "c" in p;
const isCapsule = (p: Raw): p is PartExtras & CapsuleSpec => "a" in p;

const tvBoxes = (): Array<PartExtras & BoxSpec> => [
...
const chairBoxes = (): Array<PartExtras & BoxSpec> => [
  { name: "seat", c: [0, 0.45, 0], h: [0.24, 0.03, 0.24] },
  { name: "backrest", c: [0, 0.8, -0.22], h: [0.24, 0.32, 0.03] },
  ...([[-1, -1], [1, -1], [-1, 1], [1, 1]] as const).map(([x, z]) => ({ name: "leg", c: [x * 0.2, 0.21, z * 0.2] as const, h: [0.02, 0.21, 0.02] as const })),
];
const speakerParts = (): Raw[] => [
...
const vaseParts = (): SdfPart[] => [lathePart({ name: "vase", points: [[0.18, 0], [0.32, 0.25], [0.3, 0.5], [0.14, 0.85], [0.18, 1]] })];
const ballParts = (): SdfPart[] => [mk(sphere([0, 0.4, 0], 0.4), { name: "ball" })];
const tableParts = (): SdfPart[] => [
  mk(box([0, 0.72, 0], [0.8, 0.03, 0.5]), { name: "top" }),
  ...([[-1, -1], [1, -1], [-1, 1], [1, 1]] as const).map(([x, z]) => mk(box([x * 0.72, 0.35, z * 0.42], [0.03, 0.35, 0.03]), { name: "leg" })),
];

// Raw world-space solids turned by yaw about the origin (no transform: detection measures them where they are).
const turnRaw = (list: readonly Raw[], yaw: number): Raw[] => list.map((p) => (isBox(p)
  ? { ...p, c: localToWorld([0, 0, 0], yaw, p.c), yaw: wrapAngle(yaw + (p.yaw ?? 0)) }
  : { ...p, a: localToWorld([0, 0, 0], yaw, (p as CapsuleSpec).a), b: localToWorld([0, 0, 0], yaw, (p as CapsuleSpec).b) }));
const rawThing = (list: readonly Raw[]) => ({ boxes: list.filter(isBox), capsules: list.filter(isCapsule) });

// (Raw specs -> part specs for createEntity: toPart does the rest.)
function toPartSpec(p: Raw): PartLike {
  return isBox(p) ? { box: { c: p.c, h: p.h }, name: p.name } : { capsule: { a: p.a, b: p.b, r: p.r }, name: p.name };
}

const CASES: Array<[string, () => Raw[], number]> = [
...
for (const [name, make, minConf] of CASES) {
  test(`front: ${name}, as an entity at 16 yaws (exact)`, () => {
    const base = detectFront(rawThing(make()));
  ...
  test(`front: ${name}, as raw world boxes turned to 16 yaws`, () => {
    for (const yaw of YAWS) {
      const r = detectFront(rawThing(turnRaw(make(), yaw)));
...
test("front: symmetric things come back symmetric, with low confidence -- not a made-up answer", () => {
  const cases: Array<[string, () => SdfPart[], string]> = [["vase", vaseParts, "round"], ["ball", ballParts, "round"], ["table", tableParts, "mirror"]];
...
test("front: the vocabulary reads names, camelCase and plurals, and can be extended", () => {
  ...
  const hull = { name: "hull", c: [0, 0.5, 0], h: [0.4, 0.5, 0.4] } as const;
  const porthole = { name: "porthole", a: [0, 0.6, 0.4], b: [0, 0.6, 0.42], r: 0.12 } as const;
  const ship = { boxes: [hull], capsules: [porthole] };
  assert.equal(detectFront(ship).symmetric, true, "unnamed: no front");
  // (Per call, without touching the table...)
  const once = detectFront(ship, { features: { porthole: { side: "front", weight: 1 } } });
  ...
  try {
    assert.ok(angleBetween(detectFront(ship).yaw, 0) < 5 * DEG);
...
test("front: use cases -- a seat faces out from its backrest, a screen away from its stand", () => {
  ...
  const stool: Array<PartExtras & BoxSpec> = [{ name: "seat", c: [0, 0.45, 0.05], h: [0.2, 0.03, 0.2] }, { name: "back", c: [0, 0.75, -0.17], h: [0.2, 0.3, 0.03] }];
  ...
  const mon: Array<PartExtras & BoxSpec> = [{ name: "stand", c: [0, 0.03, -0.1], h: [0.15, 0.03, 0.12] }, { name: "neck", c: [0, 0.25, -0.12], h: [0.03, 0.2, 0.02] }, { name: "panel", c: [0, 0.55, -0.05], h: [0.4, 0.25, 0.02] }];
