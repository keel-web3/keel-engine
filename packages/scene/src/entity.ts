...
// ported from the proof of concept's src/scene/entity.js.
//
...
//
// R is kit.ts rotation() (yaw about y, then pitch about x, roll about z), the
...
import { rotation } from "./kit.ts";
import type { Mat3Like, Sdf, SdfPart } from "./kit.ts";

/** Where a thing is: world = pos + R(yaw, pitch, roll) * (scale * local). */
...
/** A transform to be: every field defaulted (origin, no turn, scale 1). */
...
/** What a rotation is read from (any transform, or a bare yaw). */
...
/** A part an entity can carry: an SDF, its local bounds, and usually a name. */
export interface EntityPart extends SdfPart {
  readonly name?: string | undefined;
}

export interface Entity<P extends EntityPart = EntityPart> {
  readonly id: EntityId;
  readonly transform: Transform;
  readonly parts: P[];
  /** Sorted, unique. */
  readonly tags: string[];
  /** name -> plain data (physics, particles, a config layer...). */
  readonly components: Record<string, unknown>;
}

export interface EntitySpec<P extends EntityPart = EntityPart> {
  readonly id: EntityId;
...
/** Anything placed by a transform (an entity, or just its transform wrapped). */
export interface Placed {
  readonly transform: Transform;
}
/** Anything turned by a transform: only its rotation is read. */
export interface Oriented {
  readonly transform: Turned;
}

const IDENTITY: Mat3Like = [1, 0, 0, 0, 1, 0, 0, 0, 1];

/** Normalise a transform: { pos:[x,y,z], yaw, pitch, roll, scale }, all defaulted. */
export function makeTransform(t: TransformInput = {}): Transform {
...
export function rotationOf(transform: Turned): Mat3Like {
 ...
 */
export function createEntity<P extends EntityPart = EntityPart>(spec: EntitySpec<P> = {} as EntitySpec<P>): Entity<P> {
  const { id, transform = {}, parts = [], tags = [], components = {} } = spec;
...
export function withTransform<E extends Placed>(entity: E, patch: TransformInput): E {
  return { ...entity, transform: makeTransform({ ...entity.transform, ...patch }) };
}

export const hasTag = (entity: { readonly tags: readonly string[] }, tag: string): boolean => entity.tags.includes(tag);

/** A copy with a component set (or removed when `data` is undefined). */
export function withComponent<E extends { readonly components: Readonly<Record<string, unknown>> }>(entity: E, name: string, data: unknown): E {
...
export const byTag = <E extends { readonly tags: readonly string[] }>(entities: readonly E[], ...tags: string[]): E[] => entities.filter((e) => tags.every((t) => e.tags.includes(t)));

/** Local -> world for one point. */
export function toWorld(entity: Placed, p: Vec3Like): Vec3 {
...
export function toLocal(entity: Placed, p: Vec3Like): Vec3 {
  const { pos, scale } = entity.transform;
  const m = rotationOf(entity.transform);
  const dx = p[0] - pos[0];
  const dy = p[1] - pos[1];
...
export function dirToWorld(entity: Oriented, d: Vec3Like): Vec3 {
  const m = rotationOf(entity.transform);
  return [m[0] * d[0] + m[1] * d[1] + m[2] * d[2], m[3] * d[0] + m[4] * d[1] + m[5] * d[2], m[6] * d[0] + m[7] * d[1] + m[8] * d[2]];
}

/** World -> local for a direction. */
export function dirToLocal(entity: Oriented, d: Vec3Like): Vec3 {
  const m = rotationOf(entity.transform);
 ...
 */
export function worldSdf(entity: Placed, p: SdfPart): Sdf {
  const { pos, scale } = entity.transform;
  const m = rotationOf(entity.transform);
  const f = p.sdf;
  const inv = 1 / scale;
...
/** Anything placed with parts: an entity, as bounds and SDF queries read it. */
export interface Body<P extends SdfPart = SdfPart> extends Placed {
  readonly parts: readonly P[];
}

/** The whole entity as one world-space SDF (union of its parts). */
export function entitySdf(entity: Body): Sdf {
