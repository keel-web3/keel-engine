// Signed distance primitives on scalars -- ported from the proof of concept's
// src/core/sdf.js (NOCTURNES' sdf.js); test/*-equality.test.ts proves every
// export identical. Hot path: no arrays, no allocation.

/** A lathe profile: the outline's radii and heights, and its extent. */
...
export function sdBox(x: number, y: number, z: number, bx: number, by: number, bz: number, round = 0): number {
...
export function sdCylinder(x: number, y: number, z: number, r: number, h: number, round = 0): number {
...
export function sdCapsule(x: number, y: number, z: number, ax: number, ay: number, az: number, bx: number, by: number, bz: number, r: number): number {
  ...
  const ey = by - ay;
  const ez = bz - az;
  // (Both ends at one point is a ball: without the guard it's 0/0 and NaN everywhere. Otherwise the same arithmetic as ever.)
  const ee = ex * ex + ey * ey + ez * ez;
  let h = ee > 0 ? (px * ex + py * ey + pz * ez) / ee : 0;
  h = h < 0 ? 0 : h > 1 ? 1 : h;
  const dx = px - ex * h;
  const dy = py - ey * h;
  const dz = pz - ez * h;
...
export function sdHexPrism(x: number, y: number, z: number, r: number, h: number): number {
  const kx = -0.8660254;
  const ky = 0.5;
  const kz = 0.57735;
  let px = Math.abs(x);
 ...
 */
export function sdPolygon(px: number, py: number, xs: ArrayLike<number>, ys: ArrayLike<number>): number {
  const n = xs.length;
  const dx = px - xs[0]!;
  const dy = py - ys[0]!;
  let d = dx * dx + dy * dy;
...
export function profile(points: ReadonlyArray<readonly [number, number]>): Profile {
...
export function sdLathe(x: number, y: number, z: number, prof: Profile, round = 0): number {
...
export function sdPlanes(x: number, y: number, z: number, planes: ArrayLike<number>): number {
  let d = -Infinity;
