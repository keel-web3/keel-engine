// Screens -- ported from the proof of concept's src/core/dither.js
// (NOCTURNES' dither.js + genome.js's pairing, plus the engine's resolution
// helpers); test/*-equality.test.ts proves every threshold identical. The piece
// is drawn in continuous light and only ever SHOWN through a screen: a
// threshold map plus a small number of tone steps on the ramp.
//
...
// index. That keeps the loop from crawling and the deltas small.

import { fract, tri } from "./math.ts";

export type ScreenId = "bayer2" | "bayer4" | "bayer8" | "chunky" | "halftone" | "coarseDot" | "lines" | "diagonal" | "hatch" | "stipple" | "ign" | "checker" | "weave";
/** What a screen is made of: a lattice (ordered), dots, lines, noise, or a pattern. */
export type ScreenFamily = "ordered" | "dot" | "line" | "noise" | "pattern";
/** "grid" an axis lattice, "diag" a 45° lattice, "h"/"d"/"x" lines across, diagonal, crossed, "ign" structured noise, "none" noise. */
export type ScreenDirection = "grid" | "diag" | "tilt" | "h" | "d" | "x" | "ign" | "none";
/** A target's size class, by its short side. */
export type Band = "tiny" | "small" | "large";

/** A threshold map: `at(x, y)` -> 0..1, a pure function of the pixel. */
export type ThresholdMap = (x: number, y: number) => number;

export interface ScreenDef {
  readonly name: string;
  readonly at: ThresholdMap;
}

/** A layer screen: which map, how many tone steps on the ramp, and where the steps sit. */
...
/** A screen chosen for a target (screenForTarget): a layer screen, and why. */
export interface TargetScreen extends LayerScreen {
  readonly family: ScreenFamily;
  readonly band: Band;
}

/** What measureScreen finds in a map. */
...
export type ScreenGeometry = Pick<ScreenMeasure, "period" | "angle" | "r" | "lines">;
...
const b2 = (i: number): number => BAYER2[i]!;
const b4 = (i: number): number => BAYER4[i]!;
const b8 = (i: number): number => BAYER8[i]!;
...
function equalize(shape: ThresholdMap, w: number, h: number, tie: ThresholdMap = (x, y) => b8((y & 7) * 8 + (x & 7))): ThresholdMap {
...
const ALONG = [0, 4, 2, 6, 1, 5, 3, 7] as const;
const along = (x: number): number => ALONG[x & 7]!;
...
function dotScreen([ax, ay]: readonly [number, number], [bx, by]: readonly [number, number], w: number, h: number): ThresholdMap {
  const det = ax * by - ay * bx;
  const coords = (x: number, y: number): [number, number] => [(x * by - y * bx) / det, (y * ax - x * ay) / det];
  const shape = (x: number, y: number): number => {
    const [u, v] = coords(x, y);
    return -(Math.cos(2 * Math.PI * u) + Math.cos(2 * Math.PI * v));
  };
  const tie = (x: number, y: number): number => {
    const [u, v] = coords(x, y);
...
// repeats without a seam), made once by the proof of concept's tools/bluenoise.mjs.
// Grain with no clumps and no lattice: a stipple that reads as fine sand, not specks.
const BLUE_NOISE: Uint8Array = (() => {
...
export const SCREENS: Readonly<Record<ScreenId, ScreenDef>> = {
  bayer2: { name: "Bayer 2", at: (x, y) => (b2((y & 1) * 2 + (x & 1)) + 0.5) / 4 },
  bayer4: { name: "Bayer 4", at: (x, y) => (b4((y & 3) * 4 + (x & 3)) + 0.5) / 16 },
  bayer8: { name: "Bayer 8", at: (x, y) => (b8((y & 7) * 8 + (x & 7)) + 0.5) / 64 },
  chunky: { name: "Chunky", at: (x, y) => (b4(((y >> 1) & 3) * 4 + ((x >> 1) & 3)) + 0.5) / 16 },
  ...
  lines: { name: "Line Screen", at: (x, y) => tri(y / 3 + 0.5) * 0.9 + (b2((y & 1) * 2 + (x & 1)) / 4) * 0.1 },
...
export const SCREEN_IDS: readonly ScreenId[] = Object.keys(SCREENS) as ScreenId[];
 ...
 */
export function measureScreen(at: ThresholdMap, T = 24): ScreenMeasure {
  const vals: number[] = [];
  ...
  for (const v of vals) { const b = Math.min(15, Math.floor(v * 16)); bins[b] = bins[b]! + 1; }
  const maxBin = Math.max(...bins) / vals.length;
  const mean = vals.reduce((a, v) => a + v, 0) / vals.length;
  const varr = vals.reduce((a, v) => a + (v - mean) ** 2, 0) / vals.length || 1;
  const corr = (dx: number, dy: number): number => {
    let acc = 0;
    let n = 0;
  ...
  const shifts: Array<{ r: number; dx: number; dy: number }> = [];
  ...
  const unit = ([[0, 1, 0], [45, 1, 1], [90, 0, 1], [135, -1, 1]] as const).map(([a, dx, dy]) => [a, corr(dx, dy)] as const);
...
export const SCREEN_GEOM: Readonly<Record<ScreenId, ScreenGeometry>> = Object.fromEntries(SCREEN_IDS.map((id) => {
  const m = measureScreen(SCREENS[id].at);
  return [id, { period: m.period, angle: m.angle, r: m.r, lines: m.lines }];
})) as Record<ScreenId, ScreenGeometry>;
 ...
 * step count still land on different entries. `lums` (each ramp entry's
 * lightness) mixes two tones in light rather than in lightness.
 */
export function screenIndex(light: number, x: number, y: number, screen: LayerScreen, rampLen: number, lums: ArrayLike<number> | null = null): number {
  const steps = screen.steps;
  const v = (light < 0 ? 0 : light > 1 ? 1 : light) * (steps - 1);
  const base = Math.floor(v);
  const threshold = SCREENS[screen.id].at(x, y);
  const entry = (q: number): number => { const at = Math.round((q / (steps - 1)) * (rampLen - 1 - screen.bias)) + (q > 0 ? screen.bias : 0); return at < 0 ? 0 : at >= rampLen ? rampLen - 1 : at; };
  let f = v - base;
...
// map really runs (measureScreen measures it; the gate holds the two together):
...
export const SCREEN_KIND: Readonly<Record<ScreenId, readonly [ScreenFamily, 1 | 2 | 3, ScreenDirection]>> = {
...
export const SCREEN_PAIRS: Readonly<Record<string, number>> = {
...
export const KIND_PAIRS: Readonly<Record<string, number>> = {
...
export function screenPair(a: ScreenId, b: ScreenId): number {
  if (a === b) return 0;
...
// screen gets (screenIndex): fewer on tiny targets, where a step must cover
// several pixels to be seen. Deterministic, no state.

export const TARGET_BANDS: ReadonlyArray<readonly [Band, number]> = [["tiny", 48], ["small", 128], ["large", Infinity]];

export const TARGET_SCREENS: Readonly<Record<ScreenFamily, Readonly<Record<Band, ScreenId>>>> = {
...
export const TARGET_STEPS: Readonly<Record<Band, number>> = { tiny: 3, small: 5, large: 7 };

/** The smallest band each screen reads in (its tile against the short side). */
export const SCREEN_MIN_BAND: Readonly<Record<ScreenId, Band>> = {
...
const BAND_ORDER: Readonly<Record<Band, number>> = { tiny: 0, small: 1, large: 2 };

/** The band ("tiny" | "small" | "large") of a W x H target. */
export function bandOf(width: number, height = width): Band {
  const side = Math.min(width, height);
  for (const [band, max] of TARGET_BANDS) if (side <= max) return band;
  return "large";
}

/** A family or a screen id (any other string is refused with a RangeError). */
export type ScreenPreference = ScreenFamily | ScreenId | (string & {});
 ...
 */
export function screenForTarget(width: number, height = width, preference: ScreenPreference = "ordered"): TargetScreen {
  const band = bandOf(width, height);
  let family: ScreenFamily;
  let id: ScreenId;
  // (Looked up as plain tables, as the original does.)
  if ((SCREENS as Record<string, ScreenDef | undefined>)[preference]) {
    const pref = preference as ScreenId;
    family = SCREEN_KIND[pref][0];
    id = BAND_ORDER[SCREEN_MIN_BAND[pref]] <= BAND_ORDER[band] ? pref : TARGET_SCREENS[family][band];
  } else if ((TARGET_SCREENS as Record<string, unknown>)[preference]) {
    family = preference as ScreenFamily;
