// The port of the proof of concept's tests/core-resolution.test.mjs: the
// resolution-aware helpers, screenForTarget and rampForTarget.

import { test } from "node:test";
import assert from "node:assert/strict";
import { SCREENS, SCREEN_IDS, SCREEN_KIND, SCREEN_MIN_BAND, TARGET_SCREENS, bandOf, screenForTarget, screenIndex } from "../src/dither.ts";
import type { ScreenId } from "../src/dither.ts";
import { buildPalette, makePalette, rampBudget, rampForTarget, rampIndicesForTarget } from "../src/palette.ts";
import { createRoll, seedFromToken, stream } from "../src/rng.ts";
...
test("screenForTarget: family tables, ids kept only where they read", () => {
  ...
  for (const size of [16, 32, 48, 64, 100, 128, 200, 256, 512]) {
    for (const pref of [...Object.keys(TARGET_SCREENS), ...SCREEN_IDS]) {
      ...
      assert.ok(bands.indexOf(SCREEN_MIN_BAND[s.id]) <= bands.indexOf(s.band));
      if (pref in SCREENS) assert.equal(s.family, SCREEN_KIND[pref as ScreenId][0]);
...
test("rampForTarget: shorter ramps for tiny targets, ends kept", () => {
  ...
  // @ts-expect-error -- (a string is none of the three shapes: refused at run time too)
