...
export default defineStyledObject({
  ...
  design(J, v) {
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      const foot: Vec3 = main ? [0, H * 0.05, 0] : [dsin(yaw) * H * 0.2, H * 0.05, dcos(yaw) * H * 0.2];
      const steps = Math.max(2, Math.round(h / (r * 1.8)));
      for (let s = 0; s < steps; s += 1) {
        const off = lean * (h / steps) * s;
        const c: Vec3 = [foot[0] + dsin(yaw) * off, foot[1] + (h / steps) * (s + 0.5), foot[2] + dcos(yaw) * off];
        solids.push(solid.box(s === Math.floor(steps / 2) ? "glow" : "accent", c, [r, h / steps / 2, r * 0.8], yaw + Math.PI / 4, { name: "prism", group: `p${i}` }));
      }
      const off = lean * h;
      solids.push(solid.cone("accent", [foot[0] + dsin(yaw) * off, foot[1] + h, foot[2] + dcos(yaw) * off], r * 1.1, r * 2.4, 0, { name: "tip", group: `p${i}`, collide: false, sides: 4 }));
    }
