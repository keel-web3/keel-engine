...
export default defineStyledObject({
  ...
  design(_J, v) {
    ...
    if (form === "tripod") for (let i = 0; i < 3; i += 1) { const a = (i / 3) * Math.PI * 2 + 0.5; solids.push(solid.capsule("metal", [dsin(a) * 0.32, 0.02, dcos(a) * 0.32], [dsin(a) * 0.12, H - 0.2, dcos(a) * 0.12], 0.03, { name: "leg" })); }
    ...
    for (let i = 0; i < 5; i += 1) { const a = i * 1.3; solids.push(solid.ball("glow", [dsin(a) * R * 0.45, H + 0.02, dcos(a) * R * 0.45], [0.09, 0.05, 0.09], { name: "coal", collide: false })); }
