...
export default defineStyledObject({
  ...
  design(J, v) {
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      const c: Vec3 = [Math.sin(a) * d, 0.03, Math.cos(a) * d];
      bone(solids, [c[0] - Math.sin(yaw) * L / 2, 0.03, c[2] - Math.cos(yaw) * L / 2], [c[0] + Math.sin(yaw) * L / 2, 0.03 + J.between(0, 0.04), c[2] + Math.cos(yaw) * L / 2], 0.022);
    }
    if (bool(v["ribs"])) {
    ...
    if (bool(v["skull"])) { const a = J.between(0, 6.28); skull(solids, [Math.sin(a) * R * 0.5, 0, Math.cos(a) * R * 0.5], 0.2, J.between(-0.8, 0.8)); }
