...
export default defineStyledObject({
  ...
  design(J, v) {
    ...
    if (form === "pool") { flat(0, 0, s * 0.45, s * 0.35); for (let i = 0; i < 4; i += 1) { const a = J.between(0, 6.28); flat(dsin(a) * s * 0.35, dcos(a) * s * 0.3, s * 0.18, s * 0.14); } }
    else if (form === "splatter") { flat(0, 0, s * 0.22, s * 0.2); for (let i = 0; i < 9; i += 1) { const a = J.between(0, 6.28), d = J.between(0.2, 0.6) * s; const r = J.between(0.03, 0.07); flat(dsin(a) * d, dcos(a) * d, r, r); } }
    else {
      // (A dragged trail: smears that wander side to side, a pool at its start, drops along it.)
      const yaw = J.between(0, 3.14), sx = dsin(yaw), cz = dcos(yaw);
