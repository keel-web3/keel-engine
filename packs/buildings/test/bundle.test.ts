// The KEEL build: packs/buildings bundles into one KEEL module that reaches only
...
test(`${ID} bundles to a module that reaches only keel/object, starts on a page and builds its things there`, async () => {
  ...
  assert.deepEqual(Object.values(map).sort(), ["keel/object"]);
