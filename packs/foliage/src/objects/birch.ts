...
export default defineStyledObject({
  ...
  design(J, v) {
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      const t = trunk("paperbark", h * 0.82, r0, r0 * 0.55, { lean, leanYaw: yaw, segments: 4, flare: i === 0, from: [Math.sin(yaw) * r0 * (n > 1 ? 0.8 : 0), 0, Math.cos(yaw) * r0 * (n > 1 ? 0.8 : 0)] });
      solids.push(...t.solids);
      // (The dark marks a birch's bark is known by: two knots on its trunk, the bands are the look's.)
      for (const k of [0.3, 0.55]) { const p = t.at(k); const r = t.radiusAt(k); solids.push(solid.ball("dark", [p[0] + Math.sin(yaw + 1) * r * 0.8, p[1], p[2] + Math.cos(yaw + 1) * r * 0.8], [r * 0.45, r * 0.3, r * 0.45], { name: "knot", collide: false })); }
