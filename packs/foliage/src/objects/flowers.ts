...
export default defineStyledObject({
  ...
  design(J, v) {
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      const foot: Vec3 = [dsin(yaw) * r, r, dcos(yaw) * r];
      const head: Vec3 = [dsin(yaw) * out, h, dcos(yaw) * out];
      solids.push(solid.capsule("leaf", foot, head, r, { name: "stem", collide: false }));
      // (A leaf off the stem, low down.)
      solids.push(solid.ball("leaf", [head[0] * 0.3 + dsin(yaw + 1.6) * h * 0.08, h * 0.25, head[2] * 0.3 + dcos(yaw + 1.6) * h * 0.08], [h * 0.09, h * 0.03, h * 0.09], { name: "leaf", collide: false }));
      const s = H * 0.09;
      if (kind === "daisy") {
        ...
        const hang: Vec3 = [head[0] + dsin(yaw) * s * 1.4, head[1] - s * 0.4, head[2] + dcos(yaw) * s * 1.4];
        ...
        solids.push(solid.ball("blossom", [head[0] + dsin(a) * s * 1.1, head[1] + J.between(-0.5, 0.8) * s, head[2] + dcos(a) * s * 1.1], s * 0.5, { name: "spray", collide: false }));
      }
    }
