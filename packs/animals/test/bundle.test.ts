...
test(`${ID} bundles to a module that reaches only what it needs, and starts on a page`, async () => {
  ...
  const pack = api["pack"] as { entities: Array<{ id: string; build: (S: unknown, p: object) => { species: string } }>; attributes: unknown[] };
  assert.deepEqual([...pack.entities.map((e) => e.id)], ["dog", "cat", "fox", "bear", "rabbit", "mouse", "deer"]);
  assert.equal(pack.attributes.length, 2);
});
