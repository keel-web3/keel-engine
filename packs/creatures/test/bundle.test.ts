...
test(`${ID} bundles to a module that reaches only what it needs, and starts on a page`, async () => {
  ...
  const pack = api["pack"] as { entities: Array<{ id: string; build: (S: unknown, p: object) => { plan: string; height: number } }>; attributes: unknown[] };
  assert.deepEqual([...pack.entities.map((e) => e.id)], ["crawler", "walker", "strider", "floater", "flyer", "rider", "serpent"]);
  assert.equal(pack.attributes.length, 0);
  // It builds on the page, from its own bundle.
  const creatureOf = api["creatureOf"] as (seed: string, plan: string, o: { size: number }) => { plan: string; height: number };
  const c = creatureOf("page", "walker", { size: 2 });
  assert.equal(c.plan, "walker");
  assert.ok(c.height > 1);
});
